...
async function installBunGuard(fixture: InstallerFixture): Promise<void> {
  await executable(join(fixture.binRoot, "bun"), [
    ...
    "  exec \"$REAL_BUN_PATH\" \"$@\"",
    "fi",
    "if [[ \"$inline_source\" == *\"assertNoMisplacedConfigFiles\"* ]]; then",
    "  exec \"$REAL_BUN_PATH\" -e 'const runtime = await import(Bun.argv[1]); await runtime.assertNoMisplacedConfigFiles(\"config\");' \"$FAKE_INSTALL_RUNTIME_MODULE\"",
    "fi",
...
async function installSystemGuards(fixture: InstallerFixture): Promise<void> {
  ...
  await executable(join(fixture.binRoot, "sudo"), [
    ...
    "    target_path=\"${@: -1}\"",
    "    case \"$target_path\" in",
    "      \"$FAKE_WORKTREE\"/config/static/.bot.json.install.*)",
    "        shift",
