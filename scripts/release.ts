...
if (arguments_.includes("--help")) {
  console.log("Usage: bun run release:build -- --version MAJOR.MINOR.PATCH\n" +
  ...
  if (action !== "build" && action !== "verify" && action !== "publish") throw new Error("Expected build, verify or publish; use --help.");
  const options: Map<string, string> = new Map();
  const allowed: readonly string[] = action === "build" ? ["--version"]
