 ...
 * 全量基准被测对象的人类可读名称。
 *
 * 报告 JSON、子进程参数与性能场景继续使用稳定 id；渲染文档时把这里的动作
 * 名称放在第一行、id 放在第二行。
 */
...
const ZH: BenchmarkEntryCopy = {
  labels: {
    ...
    "sender-mixed-identity": "同群内用户与频道马甲混合发言时解析发送者身份",
    "self-sent-empty": "拒绝机器人自身的空消息",
    "self-sent-active": "机器人刚发过消息时判定一条群消息是否为自发回环",
    ...
    "temporary-whitelist-activity": "推进临时广告免检日内已达标稳态与授权边沿",
    ...
    "temporary-whitelist-write": "累计 1 条临时广告免检活动并收到 SQLite 精确回执",
    "chat-state-write": "写入 1 群状态并收到 SQLite 落盘回执",
    "chat-qa-write": "写入 1 条群问答并收到 SQLite 落盘回执",
    ...
    "cron-send-voice": "cron send_voice：合成、编码并发送 1 条语音（不含网络）",
    ...
    "quota-timestamp-window": "有配额上限的滑动时间窗口记账与过期淘汰",
    "join-timestamp-window": "有界入群滑窗的饱和记账与过期淘汰",
...
const EN: BenchmarkEntryCopy = {
  labels: {
    ...
    "sender-mixed-identity": "Resolve senders when users and channel identities interleave in one chat",
    "self-sent-empty": "Reject an empty self-sent message",
    "self-sent-active": "Decide whether a group message is a self-sent echo while the bot has recently sent one",
    ...
    "temporary-whitelist-activity": "Advance temporary-allowlist activity across its qualified steady state and grant edge",
    ...
    "temporary-whitelist-write": "Record one temporary-allowlist activity and receive its exact SQLite ACK",
    "chat-state-write": "Write one chat state and receive its SQLite durable ACK",
    "chat-qa-write": "Write one chat Q&A entry and receive its SQLite durable ACK",
    ...
    "cron-send-voice": "cron send_voice: synthesize, encode and send one voice message (no network)",
    ...
    "quota-timestamp-window": "Record into and expire a quota-capped sliding timestamp window",
    "join-timestamp-window": "Record saturation and expiry in the bounded join window",
...
const JA: BenchmarkEntryCopy = {
  labels: {
    ...
    "sender-mixed-identity": "同一 chat で user と channel 名義が交互に発言する際の送信者解決",
    "self-sent-empty": "Bot 自身からの空メッセージを拒否する",
    "self-sent-active": "Bot が直前に送信している状態で、群メッセージが自身の折り返しかを判定する",
    ...
    "temporary-whitelist-activity": "一時 allowlist の日内 qualified 定常状態と付与境界を進める",
    ...
    "temporary-whitelist-write": "一時 allowlist 活動 1 件を記録して SQLite の正確な ACK を受け取る",
    "chat-state-write": "チャット状態 1 件を書き込み SQLite 永続化 ACK を受け取る",
    "chat-qa-write": "チャット Q&A 1 件を書き込み SQLite 永続化 ACK を受け取る",
    ...
    "cron-send-voice": "cron send_voice：音声 1 件を合成・エンコードして送信する（通信を除く）",
    ...
    "quota-timestamp-window": "上限付き時刻スライディングウィンドウの記録と期限切れ削除",
    "join-timestamp-window": "有界 join ウィンドウの飽和記録と期限切れ削除",
