...
import { readBotMigrationFile } from "../migrations/botConfig/files";
...
import type { BotConfigMigrationResult } from "../migrateBotConfig";
...
export interface MigrationDeploymentOptions {
  ...
  /** 同时验证直接前序中的图库字段与单图 cron 格式；12.1.0 本身缺省这些输入。 */
  readonly extendedInputs?: boolean;
}
...
export interface MigratedDeployment {
  ...
  readonly sources: readonly BotMigrationFile[];
}

/** 建立完整 mock 备份，执行实际 CLI，核对拒绝覆盖与源哈希，再按清单组装部署目录。 */
export async function prepareMigratedDeployment({
  packageRoot, root, binary = false, historical = false, extendedInputs = false,
  ...
  const secrets: string = join(root, "source project");
  ...
  for (const directory of [config, data, images, secrets, deployedConfig, deployedData, deployedImages]) await mkdir(directory, { recursive: true });
  await Bun.write(join(secrets, "g-auth.json"), await googleAuthFixture());
  chmodSync(join(secrets, "g-auth.json"), 0o600);
  const identity: Readonly<Record<string, unknown>> = { bot_token: "123456789:migration_test_token", super_admin_user_id: 123456789 };
  await Bun.write(join(config, "telegram.json"), JSON.stringify(identity));
  ...
  for (const name of ["telegram.json", "agent.json"]) chmodSync(join(config, name), 0o600);
  ...
  if (extendedInputs) {
    await Bun.write(join(config, "cron.json"), JSON.stringify([{ name: "daily", chat_id: [7], cron: "@daily", actions: [
      { type: "send_image", payload: { url: "https://example.com/a.png", is_blurred: true } },
    ] }]));
  }
  const state: Readonly<Record<string, unknown>> = {
    global: { copy: { copiedUser: null, lastCopyTime: 12 }, assets: {
      fortuneThumbnailUrl: "https://example.com/fortune.png",
      ...(extendedInputs ? { randomImageDir: deployedImages } : {}),
    } },
    translate: { "-1001": [{ translatedUser: { id: 42, first_name: "翻译身份" }, language: "en" }] },
  };
  const backup: Readonly<Record<string, unknown>> = { global: { copy: { copiedUser: null, lastCopyTime: 6 } } };
  ...
  const sources: BotMigrationFile[] = [];
  for (const directory of [config, data, images, secrets]) {
    for await (const name of new Bun.Glob("**/*").scan({ cwd: directory, onlyFiles: true })) {
      sources.push((await readBotMigrationFile(join(directory, name)))!);
    }
  }
  ...
  const argumentsByCommand: Readonly<Record<string, readonly string[]>> = {
    "migrate:bot-config": ["--source-config-root", config, "--source-data-root", data, "--source-google-auth", join(secrets, "g-auth.json")],
    "migrate:random-image-names": ["--source-directory", images],
    "migrate:h-image-add-permission": ["--source-root", data],
  };
  ...
  for (const edge of ACTIVE_COLD_MIGRATION_EDGES) {
    ...
    run(args, true);
    const ready: BotMigrationFile | null = await readBotMigrationFile(join(output, "ready.json"));
    ...
    run(args, false);
    expect(await readBotMigrationFile(ready.path)).toEqual(ready);
    outputs.set(edge.command, output);
  }
  const identityOutput: string = outputs.get("migrate:bot-config")!;
  const botResult: BotConfigMigrationResult = await Bun.file(join(identityOutput, "ready.json")).json() as BotConfigMigrationResult;
  for (const file of botResult.outputFiles) expect(await readBotMigrationFile(file.path)).toEqual(file);
  expect(await Bun.file(join(identityOutput, "config/bot.json")).json()).toEqual({ ...identity, atmosphere: "mesugaki" });
  expect(await Bun.file(join(identityOutput, "data/state.json")).json()).toEqual(extendedInputs ? {
    ...state, global: { copy: { copiedUser: null, lastCopyTime: 12 }, assets: {
      fortuneThumbnailUrl: "https://example.com/fortune.png", randomHImageDir: deployedImages,
    } },
  } : state);
  expect(await Bun.file(join(identityOutput, "data/state.json.bak")).json()).toEqual(backup);
  expect((await readBotMigrationFile(join(identityOutput, "config/g-auth.json")))?.sha256)
    .toBe((await readBotMigrationFile(join(secrets, "g-auth.json")))?.sha256);
  if (extendedInputs) {
    expect(await Bun.file(join(identityOutput, "config/cron.json")).json()).toEqual([{ name: "daily", chat_id: [7], cron: "@daily", actions: [
      { type: "send_image", payload: { url: ["https://example.com/a.png"], is_blurred: true } },
    ] }]);
  }
  const databaseOutput: string = outputs.get("migrate:h-image-add-permission")!;
  assertMigratedDatabase(join(databaseOutput, "database/storage.sqlite"), database);
  ...
  await Bun.file(join(deployedConfig, "telegram.json")).delete();
  await copyFixtureTree(join(identityOutput, "config"), deployedConfig);
  for (const name of ["bot.json", "agent.json", "g-auth.json"]) chmodSync(join(deployedConfig, name), 0o600);
  await copyFixtureTree(join(identityOutput, "data"), deployedData);
  await copyFixtureTree(join(databaseOutput, "database"), join(deployedData, "database"));
...
export async function assertMigrationSourcesUnchanged(sources: readonly BotMigrationFile[]): Promise<void> {
  for (const source of sources) expect(await readBotMigrationFile(source.path)).toEqual(source);
}
