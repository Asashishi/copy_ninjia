 ...
 * 当前唯一受支持的那**一组**直接冷迁移边。
 *
 * schema v8 的停机备份经 migrate:ai-context 生成当前格式的独立产物，运维手工替换。
...
const ACTIVE_COLD_MIGRATION_EDGES: readonly ColdMigrationEdge[] = [{
  command: "migrate:ai-context",
  invocation: "bun scripts/migrateAiContext.ts",
  entryPath: "scripts/migrateAiContext.ts",
  scope: "schema v8 → schema v9 with AI context, persona and permission storage",
}];
...
export async function collectColdMigrationProblems(
  ...
  if (IDENTITY_DATABASE_SCHEMA_VERSION !== 9) {
    problems.push("AI context cold migration must map schema v8 to the current SQLite schema");
  }
