...
- **起きていない状態変化を応答が報告してはいけません。** `/init`、`/ai_chat`、`/ad_detect`、`/flood_control`、`/antiraid`、`/translate` の 6 つの switch command は書き込み前に必ず元の値を読み、同じ状態で繰り返し実行した場合は「元からそうです」と言い切らなければなりません。変更直後の文をそのまま流用すると、管理者は最初の実行が効いたのかどうか判断できません。4 つの結末の文言は `ToggleCommandTexts`（`packages/types/commands.ts`）という **4 項目すべて必須**の構造に収め、選択は `toggleReplyText` が行います。「on」「off」の 2 文しか用意しない新しい switch command は compile できません。`/quiet`、`/unquiet`、`/white`、`/permission` は同じ方針の既存実装です。
  ...
  **`/init disable` も永続化を teardown より先に行います**（`runChatToggleCommand` と同じ順序）。`teardownChatRuntime` には不可逆な永続化処理（aiChat owner の durable な記憶削除、translate owner のセッション削除）が含まれるため、逆順にすると書き込み失敗の瞬間に「ディスク上の switch はまだ on、このチャットの AI 記憶はすでに消えている」になります。総 switch のその一回は従来どおり throw します——まだ何も書いていないので、再配信の回でも `wasEnabled` は true のままで、上記の「もともと off だった」という曖昧さは生じません。`teardownChatRuntime` が同期的に消す永続フィールドは `isProxySendEnabled` だけ（他の owner はプロセス内状態のみ、あるいは translate のように自分で永続化します）なので、二回目の書き込みは実際に代理送信セッションが開いている場合にのみ行い、その書き込みは teardown と同じ降格経路を共有して「片付け切れなかったものがある」と応答し、offset を保留しません。
...
- chat runtime teardown の 7 つの固定 owner（`copy`、`translate`、`gag`、`qa`、`wed`、`aiChat`、`antiRaid`）callback は `packages/cache/main/chatTeardown.ts` が保持します。上位ドメインは業務依存を持たない leaf `packages/infra/chatTeardownRegistry.ts` を通じて逆向きに登録します。`packages/infra/chatTeardown.ts` は cleanup の統括だけを担い、`packages/infra/botAdmin.ts` はその統括境界にのみ依存し、`commands/`、AI、Anti-Raid の業務モジュールへ static dependency を持ってはいけません。
...
- 候補の情報源は `memory/wed` のメンバー集合だけです。抽選した ID には `getChatMember` を 1 回だけ発行して身分を取得します（図注と公開アバターのフォールバックが必要とします）。在室判定も Bot 除外も行わず、集合へ書き戻しもしません——Bot は書き込み側で弾いており、退室したメンバーは退室更新と日次再確認が整理します。`readCurrentAvatar` は 3 状態を返し、「利用可能なアバターが無いと確定した」場合だけが 1 回の抽選あたり 8 候補の枠を消費します。メンバー照会やアバター照会が完了しなかった場合は枠を消費せず、3 回積み上がった時点でその回を諦めます。変更では現在の相手を除外します。図注の二人の名前はユーザーメンションのエンティティで表示します。候補照会、画像ダウンロード、画像送信・編集は 30 秒の要求予算を共有し、update と session の取消にも従います。画像バイトは session、ファイル、DB に保存しません。Telegram download gate、有界読み取り、公開 asset domain 検証を再利用し、Bot のアバター設定 API は呼びません。
