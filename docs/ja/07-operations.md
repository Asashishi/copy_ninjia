...
- **`state.json` + `state.json.bak`**
  ...
  - **ランダム画像ディレクトリ**（`global.assets.randomImageDir`、既定 `images`、データルート
    ...
    `jpg`/`jpeg`/`png`/`webp` を運用側で置くか、グループで `/h_image add` を使って追加します
    （ファイル名は Telegram の `file_unique_id` と拡張子。ドットで始まる `.h_image-add-*` は書きかけの
    一時ファイルで、残っていれば削除して構いません）。サービスアカウントが読める必要があり、
    `/h_image add` を使うなら書き込みもできる必要があります。
    ディレクトリが無ければサービスが起動時に 0755 で作成します。画像の追加・削除は再起動不要で、
    パスの変更は他の項目と同じく停止中に行います。このキーを知らない旧版へ戻す前に、
    `state.json` の主・副から削除してください。
...
唯一の入口は [`scripts/migrateHImageAddPermission.ts`](../../scripts/migrateHImageAddPermission.ts) です。直前の migration が出力した厳密な schema v10 系譜（12.x リリースの形式）だけを受け入れ、schema v11 を出力します。古い版は次節の手順で先に v10 まで段階的に更新してください。未知の系譜と変換済み v11 は拒否します。本番起動は現行形式だけを検証し、migration を行いません。
...
- **運勢結果と receipt key が不整合**
  ...
  - **対応**：Bot を停止し、同じ整合時点の `memory/luck/` 全体を復元。
    key だけを削除・再生成しない。
- **state の主ファイルまたはバックアップが不正**
  - **原因**：`state.json` または `state.json.bak` を解析できないか、現在の schema に一致しない。
  - **対応**：サービスを停止したまま両方の原本をバックアップし、エラーに示されたファイルパス、
    フィールドパス、期待される形式に従って修正して再検証します。
    ランタイムは不正ファイルの全バイトを保全して起動を拒否し、`*.corrupt` ファイルを生成しません。
...
- `Cron task "<name>" action #<n> (<type>) failed in chat <id> after <k> attempt(s)`：`chat_id: "all"` のタスクがあるグループで最終的に失敗し、そのグループの残りの動作だけを飛ばしました。他のグループには通常どおり送ります。原因の読み方は上と同じです。`Cron task "<name>" skipped <n> chat(s) without send permission.` は通常ログで、Bot に送信権限がないか照会に失敗したためにその回で飛ばしたグループがあったことを示します。
