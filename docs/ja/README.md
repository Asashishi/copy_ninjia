...
`/h_image` はランダム画像ディレクトリ（`state.json` の `global.assets.randomImageDir`、既定はデータルート直下の `images/`、起動時に自動作成）から一様に 1 枚引いてグループへ送り、画像は残ります。対象は `jpg`、`jpeg`、`png`、`webp` だけで、画像の追加に再起動は不要です。
