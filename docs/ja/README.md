...
<td align="left" valign="top" width="33%">
  <p><b>🎨 人設と通知の口調</b><br>
  <sub>群ごとに AI 人設を設定でき、Bot の通知は雌小鬼・普通から選択。専用人設のある群では普通の口調を使います。</sub></p>
</td>
</tr>
<tr>
<td align="left" valign="top" width="33%">
...
BotFather で `/setcommands` を実行する必要はありません。Bot は起動時に設定された通知口調でコマンドメニューを登録し、カスタムペルソナを設定したグループでは通常版メニューに切り替えます。メニューはグループチャットにだけ表示されます。プライベートチャットはスーパー管理者の `/send` しか受け付けないため、メニューを表示しません。
