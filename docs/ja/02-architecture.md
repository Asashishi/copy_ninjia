...
返信がトリガーされると、ローリングメモリは前節で説明した 3 部構成の Gemini 入力に組み立てられ、`googleSearch` とカスタムツールとともに Gemini へ送られます。モデルは 1 ラウンド内で複数回ツールを呼び出せますが、いずれも Telegram を直接操作せずメインスレッドのプロキシ経由で実行されます。
