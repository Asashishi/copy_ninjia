...
`bun run test:coverage`：**1581 tests / 169 files / 29945 `expect()` calls**。全ソースコードの**関数カバレッジは 95.15%、行カバレッジは 96.54%**です。ルート README の Coverage badge は行カバレッジを表示します。
...
- **中国語の文字列リテラル数**（現在およそ 581 ソース行 / 65 ファイル）：3 言語 README の「言語について」注記と、3 言語の [06 よくある変更手順](06-modification-guide.md)「i18n を行わない」節に出てきます。ユーザー向け文言を増減したら数え直します。コメントを除き、文字列または template literal を含むソース行を数えます。
