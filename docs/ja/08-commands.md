...
<tr><td><code>/h_image</code></td><td align="center">メンバー</td><td>ランダム画像ディレクトリから一様に 1 枚引いてグループへ送信。引数なし。画像は残し、失敗時の案内は 30 秒後に削除</td></tr>
...
グループで `/init enable` を実行済みなら、誰でも `/h_image`（引数なし）を送れます。Bot は `state.json` の `global.assets.randomImageDir`（既定はデータルート直下の `images/`、無ければ起動時に自動作成）から 1 枚を一様に引き、コマンドへの返信としてグループへ送ります。フォーラムグループではコマンドのあるトピックに送ります。
...
- **同時実行**：コマンドは受理後すぐに返り、更新処理を止めません。抽選とアップロードは主スレッドの実行器で行い、同時実行は最大 2 件、待機枠は 16 件で、満杯なら「後でもう一度」と返します。
