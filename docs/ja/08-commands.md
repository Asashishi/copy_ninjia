...
運用者が `config/cron.json` を設定します。ホットリロードに対応し、群内の編集コマンドはありません。指定チャット、送信権限のある有効な全群、または一部を除いた群へ文章・画像・ファイルを送れます。時区・一度限りの実行・ランダム間隔に対応します。固定画像は `url` またはファイル `path` の 1〜10 項目の配列で、2〜10 枚は先頭だけ説明付きのアルバムになります。ランダム画像は毎回 1 枚で独立ディレクトリも指定でき、`is_blurred` は全画像のネタバレ防止を制御します。
