...
セッションと画像はメモリ内だけに置き、画像は現在の処理中だけ、セッションは一群あたり最大 512 件保持します。結果はボタンと chat teardown が所有し、固定 30 秒削除の対象にはしません。使い方や失敗の通知は共通の 30 秒削除を使います。`/init disable` と chat runtime teardown は操作を取り消し、メンバー記録は保持します。再起動後の旧ボタンは `/wed` の再送を案内します。
