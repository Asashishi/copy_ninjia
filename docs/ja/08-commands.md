...
- **群内通知の口調とメニュー**：Bot がいる群で専用人設を設定した場合だけ、コマンド通知・権限 help・ボタン・自動通知を普通版にします。未設定や削除後は既定の雌小鬼版です。AI switch とは独立し、`/prompt` の永続化完了後に本群の Telegram メニューを同期します。通常の inline 運勢は送信先群 ID がないため既定の雌小鬼版を使います。メニューは全グループチャットのスコープ（`all_group_chats`）にだけ登録し、起動時に既定スコープを消すため、プライベートチャットにはメニューが出ません。プライベートチャットのコマンドは、スーパー管理者の `/send` 以外すべて返信しません。
...
毎日東京時間 00:00 に共通 Bun cron が主スレッドへ通知し、保存済みの全メンバー集合を全群合計で毎秒最大 5 ID の速度で再確認します。対応する群からの退室が確定した ID は元の Set から削除し、上記のバッチ経路で保存します。照会失敗や timeout では記録を保持します。未完了の走査には次回を重ねず、停止時は再確認を取り消して残りの変更を送信します。プロセス再起動後は次の 0 時通知を待ちます。

セッションと画像はメモリ内だけに置き、画像は現在の処理中だけ、セッションは一群あたり最大 512 件保持します。操作キャッシュは 1,024 件の LRU で、コマンドとボタンの参照で利用順を更新し、満杯での追加時は最も長く使われていない群の操作を除去して結果を清掃します。除去後の旧ボタンは無効です。メンバー記録は独立して保持し、メンバー表は 25 群で満杯となり新規群を拒否するため、通常運用は引き続き最大 25 群です。結果はボタン・LRU eviction・chat teardown が所有し、固定 30 秒削除の対象にはしません。使い方や失敗の通知は共通の 30 秒削除を使います。`/init disable` または Bot の退室時には操作を取り消し、その群のメンバー記録と永続化ファイルを削除します。管理者権限の喪失時はメンバー記録を保持します。再起動後の旧ボタンは `/wed` の再送を案内します。
...
- **取得元**：毎回その場で照会します。ユーザーはこのグループでのメンバー状態、チャンネルやグループはチャット情報、Bot 自身は起動時の自身の情報を使います。照会に失敗したら Bot が覚えている身元に戻り、何もなければ「見つからない」と返します。アバターは `/wed` と同じ読み取り（ユーザーのアバターは file_id で再利用、チャンネルのものはダウンロードしてアップロード、t.me ページでの補完あり）で、取れなければ画像を付けません。
...
グループで `/init enable` を実行済みなら、誰でも `/h_image`（引数なし）を送れます。それ以外の引数には使い方の案内だけを返します。Bot は `state.json` の `global.assets.randomImageDir`（既定はデータルート直下の `images/`、無ければ起動時に自動作成）から 1 枚を一様に引き、コマンドへの返信としてグループへ送ります。フォーラムグループではコマンドのあるトピックに送ります。

- **画像の対象**：ディレクトリ直下の `jpg`、`jpeg`、`png`、`webp` だけで、隠しファイル・サブディレクトリ・シンボリックリンクは数えません。使うたびに列挙し直すため、画像の追加・削除に再起動は不要です。
- **保持**：送った画像は残し、30 秒削除は掛けません。引数付きの使い方案内、混雑時の案内、そしてディレクトリが無い・画像が無い・引いた画像が 10 MB を超える、の 3 種の失敗案内は 30 秒後に削除します。
...
- **名前と重複**：ファイル名は Telegram の `file_unique_id` とファイル先頭から判定した拡張子（`.jpg`/`.png`/`.webp`）です。同じ `file_unique_id` の画像がすでにあればダウンロードせずに飛ばします。対応外の形式や 10 MB を超える画像は追加しません。
...
- **返信**：返信していない、返信先に画像がない、権限がない場合は案内を 1 行返します。終わると、追加した枚数と追加前のライブラリの枚数（`/h_image` の抽選候補と同じ数え方）を 1 行で返し、既存のため飛ばした画像や失敗した画像があればその枚数も添えます。案内はすべて 30 秒後に削除します。
