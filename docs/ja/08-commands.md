...
- **群内通知の口調とメニュー**：Bot がいる群で専用人設を設定した場合だけ、コマンド通知・権限 help・ボタン・自動通知を普通版にします。未設定や削除後は既定の雌小鬼版です。AI switch とは独立し、`/prompt` の永続化完了後に本群の Telegram メニューを同期します。通常の inline 運勢は送信先群 ID がないため既定の雌小鬼版を使います。
