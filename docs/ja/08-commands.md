...
翻訳は文字メッセージだけを扱い、全体の copy 対象、5 分間のクールダウン、アバター操作から独立しています。`isCanControllTranslatePermission` を持つ identity が先に `/translate enable` を実行します（既定は無効）。有効な `g-auth.json` が必要です。翻訳対象が別の Bot の場合は、@BotFather でこの Bot の Bot-to-Bot Communication Mode を有効にしてください。無効のままでは相手の通常メッセージが届きません（[01 環境構築](01-getting-started.md) を参照）。
