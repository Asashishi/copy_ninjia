...
セッションと画像はメモリ内だけに置き、画像は現在の処理中だけ、セッションは一群あたり最大 512 件保持します。操作キャッシュは 1,024 件の LRU で、コマンドとボタンの参照で利用順を更新し、満杯での追加時は最も長く使われていない群の操作を除去して結果を清掃します。除去後の旧ボタンは無効です。メンバー記録は独立して保持し、メンバー表は 25 群で満杯となり新規群を拒否するため、通常運用は引き続き最大 25 群です。結果はボタン・LRU eviction・chat teardown が所有し、固定 30 秒削除の対象にはしません。使い方や失敗の通知は共通の 30 秒削除を使います。`/init disable` と chat runtime teardown は操作を取り消し、メンバー記録は保持します。再起動後の旧ボタンは `/wed` の再送を案内します。
