...
<tr><td><code>/set_qa</code></td><td align="center"><code>isCanControllQaPermission</code></td><td>「設定テキスト / 設定回答」の 2 ボタン form を表示し、両方揃うとこの chat の Q&amp;A を 1 件登録（最大 5 件）</td></tr>
<tr><td><code>/query_qa</code><br><code>/query_qa &lt;質問文&gt;</code></td><td align="center">グループメンバー</td><td>この chat の Q&amp;A を JSON code block で一覧表示、または 1 件だけ照会。board は長期保持し、該当なしの通知は 30 秒後に削除</td></tr>
...
- **chat Q&A**：`/set_qa` の form は inline prefill でテキストを集めるため、2 つの制約があります。**channel の皮（sender_chat）は一律拒否**します。Telegram は皮の下が誰かを本プロセスに一切伝えないため、維持権限を与えることはこの chat の Q&A をその皮を着られる全員に渡すのと同じで（`/permission`・`/white` が現在の chat を対象として拒否するのと同じ理由）、そもそも皮の身分は inline を使えません。forum では **General も他の話題と同様に使えます**。inline 入力が無いのは左端の「すべて」集約ビューだけで、そのビューは bot 側から General と区別できません（どちらの message にも `message_thread_id` が付きません）。したがって判定は行いません。そこでボタンが反応しない場合は、任意の話題に入って再試行してください。form は chat ごとに 1 つ、10 分で失効します。実際の書き込み資格は結果が着地した時点で送信者に対して再検証し、「誰が form を開いたか」では決めません。`/init disable` と chat teardown が片付けるのは未完了の form だけで、**登録済みの Q&A は database に残ります**。運用者が登録した設定だからです。`/init enable` し直せばそのまま有効で、削除するには `/remove_qa` を使います。
- **Q&A の直接応答**：chat が `/init enable` 済みで、メッセージ本文が登録済みの質問と**一字一句同じ**場合、Bot は AI を介さず即座に回答します。@ / 返信 / ランダム発言といった通常のトリガー条件にも縛られません（Bot への返信や @ mention を含む）。先頭の `@bot ` は比較前に取り除きます。意味は近いが文字列が異なる質問はこの経路を通らず、AI ラウンド内の `group_qa_query` と `group_qa_answer` という 2 つの照会 tool に委ねます。どちらもラウンドの可視アクション予算を消費せず、chat に登録が無ければ tool 自体が存在しません。
