...
<tr><td><code>/set_qa</code></td><td align="center"><code>isCanControllQaPermission</code></td><td>Posts a two-button form (set question / set answer); filling both registers one Q&amp;A pair for this chat, up to five</td></tr>
<tr><td><code>/query_qa</code><br><code>/query_qa &lt;question&gt;</code></td><td align="center">Chat member</td><td>Lists this chat's Q&amp;A as a JSON code block, or just the one pair; the board is retained, a not-found notice self-deletes after 30 seconds</td></tr>
...
- **Chat Q&A**: the `/set_qa` form collects its text through inline prefill, which imposes two hard constraints. **Channel identities are refused** — Telegram never tells this process who is behind the mask, so granting it maintenance rights hands this chat's Q&A to anyone who can wear that skin (the same reason `/permission` and `/white` refuse the current chat as a target), and a masked identity cannot use inline anyway. In a forum, **General works like any other topic** — the only view without inline input is the leftmost aggregated "All", and that view is indistinguishable from General on the bot's side (neither carries a `message_thread_id`), so no check is attempted: if the buttons do nothing there, enter any topic and try again. The form is one per chat and expires after ten minutes; the actual right to write is re-checked against the sender when the result lands, not decided by who opened the form. `/init disable` and chat teardown only collect an unfinished form — **registered Q&A rows stay in the database**, because they are configuration the operator entered: they take effect again after `/init enable`, and removing one still requires `/remove_qa`.
- **Direct Q&A answers**: when the chat is `/init enable`d and a message's text matches a registered question **exactly**, the bot answers straight away without involving the AI and regardless of the usual @/reply/random triggers — replies to the bot and @mentions included. A leading `@bot ` is stripped before comparing. Semantically close but textually different questions do not take this path; they are left to the `group_qa_query` and `group_qa_answer` query tools inside an AI round, neither of which consumes the round's visible action budget and neither of which exists when the chat has no entries.
