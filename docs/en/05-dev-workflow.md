...
| `bun run test:random` | Run the whole suite in a fixed-seed random order to expose cross-test residue. Reset stubs with `mockReset()`: `mockClear()` leaves queued `mockResolvedValueOnce` values in place, so an unconsumed one leaks into the next test; after a reset every stub must have its implementation reinstalled |
...
| `bun run perf:review` | Targeted review: 12 existing hot paths, 7 AI reply/payload scenarios, two complete command chains, and real Disk I/O Worker pressure; three independent rounds per item, selected with `--hot-paths` / `--ai` / `--chains` / `--worker`; text sanitization and cooldown reviews run only with explicit `--text` / `--cooldown` |
...
- **The coverage denominator includes all source code**: `bun run check` adds every production runtime module to the denominator. Modules untouched by any test count as 0% covered. Both function and line coverage must remain at least 95%, so adding an untested module directly lowers global coverage.
...
- **Separate type imports**: source code, scripts, and tests use standalone `import type` declarations. ESLint's `no-restricted-syntax` rejects inline type specifiers such as `import { value, type Shape }`. `test/scripts/typeImportConventions.test.ts` checks accepted and rejected forms in all three file groups and verifies that the `Promise.all` restriction remains active.
...
- **Convention checks**: `check:conventions` checks code placement, local Markdown targets, whether files named by Markdown directory listings written as "`<dir>/` (`a.ts`, `b.ts`)" still exist (skipped when the directory name does not resolve uniquely), executable permissions on tracked non-script files, constants, cache ownership, and Worker/Telegram boundaries against the real thread module graph. It also statically checks that every timer handle installed under `packages/workers/` is `unref()`ed, Node-compatibility imports in production code, scripts, and tests, the allowed `Buffer` methods, process arguments that must use `Bun.argv`, Telegram cleanup and retention exemptions, active cold-migration entries, the fault-injection manifest, direct root `package.json` declarations for runtime bare imports (a package reachable only through a hoisted transitive dependency fails; runtime builtins and type-only references are excluded, and package subpaths map to their package), all 14 declared coverage locations, and the trilingual performance record. Cross-references written in comments as "see `<module>.ts`'s `<symbol>`" are checked the same way: it fails when the named module no longer declares or re-exports that symbol (`export *` compatibility entries are expanded one level). `check:coverage` performs a separate real coverage run to ensure the declarations have not drifted together.
  Module-level literal constants and literal combinations belong in domain `consts`; function composition and cache ownership are checked separately. Module-level Map, Set, WeakMap, WeakSet, AsyncLocalStorage, and holder objects may be declared only under an owned `packages/cache/` directory. Builtin Node modules share one allowlist with or without the `node:` prefix. Dynamic loading, re-exports, `require`, `process.hrtime` / `nextTick`, and destructuring are checked too; type-only declarations are excluded from runtime checks.
...
Installer isolation also covers a missing or mismatched unit data root, rejection of `EnvironmentFiles` and relevant `PassEnvironment` / `UnsetEnvironment`, the post-start `NRestarts` baseline and rejection of decreasing counts, and mode preservation when refilling existing configuration. Fixtures intercept all system commands; failed preflight must precede configuration, unit, and runtime-data writes.

`test/scripts/installMigration.test.ts` verifies the complete path from 12.1.0 mock backups through cold migration, manual placement of mapped outputs, source installation, and real startup; it also rejects an unmigrated identity entry. During build verification, `scripts/checkBinary.ts` performs corresponding checks on packaged migration tools, binary installation, and startup without system Bun in the target process. Both use `scripts/fixtures/migrationDeployment.ts`, covering both valid schema v10 lineages, nonempty WAL, every database business table, both state copies, Google credentials, deployment configuration, and image-library contents. Source hashes, modes, ownership, and link topology stay unchanged. Database migration adds only the declared permission bit, updates the version, and appends a lineage entry; other business contents stay unchanged.

Credentials use temporary generated RSA keys. `test/scripts/migrateBotConfig.test.ts` also checks exact BOM preservation, invalid or missing credentials, conflicting sources, and refusal to overwrite outputs. Source migration installation tests are included in fault injection. Run `bun test --isolate test/scripts/installMigration.test.ts test/scripts/migrateBotConfig.test.ts` for the source path; `bun run build -- --version <tag>` verifies the binary path.
...
`bun run test:fault-injection` covers application/Worker lifecycles, lockdown restoration, reply capacity and cancellation, credential snapshots, shutdown draining of Telegram outbound requests and delayed deletions, chat teardown and the join-log durability barrier, Anti-Raid task draining and verification recovery, duplex Worker rebuild cancellation, and Disk I/O inspection, atomic writes, and recovery failures. The full manifest is the script in [`package.json`](../../package.json). `check:conventions` detects missing tests through resolved references to registered harnesses and production recovery/lifecycle boundaries, including static value imports, dynamic imports, value re-exports, and directory `index.ts` entries. Type-only references are excluded; empty declarations retain their side effects, and unrelated same-name modules do not match. A mixed-purpose compatibility entry can restrict its boundary to specific exports (`infra/telegram/index.ts`, `actions.ts`, and `actions/messageLifecycle.ts` count only the delayed-deletion flush/drain exports). Namespaces are judged by their property accesses, and spreading one into an object-literal stub does not count; a namespace passed on as a whole, `export *`, and dynamic imports whose result is not bound cannot prove their scope and always count. Changes to persistence, shutdown, or Worker lifecycles in [04 Runtime invariants](04-invariants.md) must pass this suite.

`test/workers/antiRaid/verificationWelcome.test.ts` exercises the real duplex protocol, main-thread temporary-message boundary, and deletion owner, with an SDK transformer handling Telegram output. It covers all four welcome texts, reply targets, lost responses, cancellation, Worker teardown/replacement, and send/transport failures. Assertions check single cleanup ownership, unreferenced timers, and the ordering of subsequent effects. The file belongs to both the full test suite and fault injection.

`/wed` interaction regressions cover the 1,024-entry LRU, command and button recency, cancellation of queued and active interactions on eviction, late-result cleanup, continued cleanup after individual deletion failures, isolation from update cancellation, and shutdown draining. The authoritative member table separately verifies rejection at 25 groups. Persistence regressions cover per-group set identity, the 150,000-member cap, departure capacity reuse, dirty TTL/count thresholds, silent no-ops, delivery failures, Worker recovery watermarks, shutdown flush, and invalid files refusing startup before networking while retaining their bytes. Draw regressions additionally confirm that avatars are read by ID, identity comes from the `getChat` private-chat profile, and `getChatMember` is no longer called. `test/app/registerHandlersDispatch.test.ts` also verifies that rejected initialization gates still remove departed IDs without admitting business handlers. Performance checks reuse `wed-member-hit`, `wed-member-growth`, `wed-member-churn`, `wed-member-chat-switch`, and `registered-middleware`; `wed-member-churn` checks that full sets reject new IDs and retain existing members.
...
`scripts/perf/hotPaths/gateResult.ts` strictly parses the calibration stored in [`performance-result.json`](../../performance-result.json). Before convention checks proceed or hot-path children start, `gateRuntime.ts` compares `packageManager`, the current Bun version/revision, and the calibrated build; mismatches require remeasurement. The record contains process counts, per-scenario latency and GC pause measurements, and hard RSS/retention limits. Historical `fullSuite` results retain their own timestamps and Bun builds.

Each `steadyProfile` child explicitly enables `BUN_JSC_logGC=1`. `hotPaths/gcProfile.ts` sums JSC `p=…ms` pause phases only inside the formal loop boundaries and divides by that window's monotonic elapsed time. The startup handshake, one complete window, and pause format must match; missing or unknown formats fail. Zero is valid only with complete valid logs and no pauses. Sampling profiles still report JIT tiers. Forced GC in `retained` runs outside the timed window and is excluded from this ratio. Per-scenario calibration stores pause measurements from at least three independent processes. All scenarios share a GC pause limit selected by the process's available CPU count: 25% for 4 or more CPUs, 30% for 2–3 CPUs, and 35% for 1 CPU. A reading equal to the limit passes; a higher reading fails. `HOT_PATH_GC_CPU_BUDGETS` in `packages/consts/performance.ts` defines the policy. At startup, the gate reads available parallelism through `node:os.availableParallelism()` and selects the budget; Linux CPU affinity restrictions affect this count. The output records the selected CPU count and common limit in `availableCpuCount` and `thresholds.maxGcPausePercent`.

Profile output from `perf:isolated-hot-path --profile` and `perf:review` provides JIT and sampling diagnostics, not GC pause ratios. Use `perf:hot-path-gate` for GC measurements. `perf:disk-transport` and `perf:review --text` also parse GC logs in the parent and return a separate `gcProfile` for each round.
...
`--cooldown` explicitly selects five production cooldown scenarios: `cooldown-hit`, `cooldown-renew`, `cooldown-growth`, `cooldown-saturated`, and `cooldown-expiry`, covering existing-key hits, single-key renewal, table growth, full-table rejection, and batch expiry. Capacity and window length come from production constants. Each scenario uses three independent timing and three profile processes, asserts admission counts, and observes production JIT probes. Growth resets before each warmup and formal sample, outside the timed interval. Normal runs report latency, retained heap, and RSS; profiles provide JIT samples but no GC pause ratio. This mode is excluded from the default review and full suite and does not change hard-gate thresholds.

`--text` runs only when requested explicitly and is not part of the default full review. Each of its 36 scenarios runs three independent processes and calls production `sanitizeInline` and `buildBufferedMessage`; message construction uses the default clock and time formatting, and results are retained in a window of the compaction batch size. Fixtures rotate Chinese, English, and emoji text at 6:3:1 and cover bodies of 8–4,096 code units, long-body shares of 1%–75%, head/middle/tail line breaks, dense whitespace, mixed layouts, and reply references. Each child performs the same memory read during warmup as in formal sampling, waits for three consecutive samples with unchanged JIT probes, then takes nine samples. It reports median latency, peak heap and RSS deltas, retained heap, JIT tiers, and the GC pauses parsed by the parent; each scenario summarizes the mean, range, CV, and JIT stability of the three round medians.
...
All data is written under the repository-root `performance/` directory (already in `.gitignore`), configuration is read from `config_example/`, and each round's tree is removed afterwards; nothing should remain there once the run ends. The parent process imports no production implementation module, so nothing reaches the real data root through a production write path. Creating directories, copying, writing files, and deleting additionally share one boundary (`scripts/perf/fullSuite/mockRoot.ts`): the path is first checked lexically to fall under `performance/`, then every **already existing** component between the repository root and the target is inspected in turn, and any component that is a symbolic link is refused. Deletion checks only the parent chain, so a leaf that is itself a symbolic link is merely unlinked without touching its target; the mock root itself is never deleted. Adding `--write-doc` writes both the blocks in `docs/{cn,en,ja}/09-performance.md` and `fullSuite.lastRun` in `performance-result.json`; the figures and the meaning of each section live in [09 Performance Benchmark](09-performance.md).
...
# Run on clean master after squashing and pushing master and the annotated tag.
bun run release:publish -- --version "$RELEASE_VERSION" --platforms "$RELEASE_PLATFORMS" --notes-file /tmp/release-notes.md
```

Asset names are `copy-ninjia-<platform>.tar.gz` and `copy-ninjia-<platform>.tar.gz.sha256`. Binary installation downloads these Release assets and verifies SHA-256; the target machine does not check out or build source code.
