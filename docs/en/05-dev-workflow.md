...
| `bun run perf:review` | Targeted review: 12 existing hot paths, 7 AI reply/payload scenarios, two complete command chains, and real Disk I/O Worker pressure; three independent rounds per item, selected with `--hot-paths` / `--ai` / `--chains` / `--worker`; the text-sanitization review runs only with an explicit `--text` |
...
`bun run test:coverage`: **4531 tests / 389 files / 158942 `expect()` calls**; full-source **function coverage 97.23% / line coverage 98.03%**. The Coverage badge in each project README displays line coverage.
