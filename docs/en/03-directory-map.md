...
| `packages/commands/` | Explicit command handling, one command per file; the permission and configuration gates shared by toggle commands live in their own files | `copy.ts`, `block.ts`, `cjkAction.ts`, `send.ts`, `targetResolution.ts`, `superAdminToggle.ts`, `configGate.ts` |
...
| `packages/aiChat/` | Main-thread AI chat proxy: Worker supervision entry point, memory mirror, and the single decision point for whether AI chat is live | `index.ts`, `memoryMirror.ts`, `availability.ts` |
| `packages/antiRaid/` | Main-thread Anti-Raid proxy: Worker supervision entry point, lockdown recovery, pending-verification mirror intake, the blocklist join decision, ad-detection submission/disposal, and flood-counting submission | `index.ts`, `lockdownMirror.ts`, `verificationMirror.ts`, `blocklistGuard.ts`, `adDetect.ts`, `floodControl.ts`, `memberFacts.ts` |
...
| `packages/config/` | Strict schemas and lazy loading for `config/*.json`, plus per-feature readiness verdicts | `stickers.ts`, `reactions.ts`, `mood.ts`, `adSamples.ts`, `readiness.ts` |
...
5. **Is it side-effecting code or orchestration?** → place it with its owner: commands in `packages/commands/`, automatic behavior in `packages/auto/`, Worker-internal logic in `packages/workers/<domain>/`, AI capabilities in `packages/ai/`, and process-level infrastructure in `packages/infra/`.
...
Watch out for shared domain code such as `packages/ai/`: if a pure function used only by the main thread lives in the same file as a Worker-owned cache, importing that function from the main thread instantiates the cache there too. [`packages/ai/stickers/describe.ts`](../../packages/ai/stickers/describe.ts) is the worked example — it was split out of `sets.ts` precisely so the main-thread message pipeline can format sticker descriptions without touching the AI Worker's sticker-set cache.
