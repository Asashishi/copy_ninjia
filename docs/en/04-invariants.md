...
- `config/whitelist.json` and `config/blocklist.json` are not optional files covered by the previous rule. The former controls synchronous authorization and insider protection; the latter is a static enforcement boundary.
  ...
  `/permission query` and `/permission help` are read-only entry points. An allowlisted user or channel reads the same in-memory snapshot under its command-sender identity; `query` returns only that identity's complete permissions after defaults have been applied, accepts no target, and performs no write. `help` also preserves the super administrator's existing access. Every permission mutation remains super-administrator-only. In groups, `help` remains permanently while `query`, refusals, and usage hints use the shared 30-second cleanup.
...
- **Flood counting and enforcement both live in the Anti-Raid Worker; the main thread only applies synchronous gates and makes one best-effort `post`.** The feature defaults off per chat and enters counting only when `ChatState.isFloodControlEnabled === true`. The super administrator or an allowlisted identity with `isCanControllFloodControlPermission` may persistently change it through `/flood_control enable|disable`; disabling also clears that chat's live windows. When one member posts `FLOOD_MESSAGE_LIMIT` messages (currently 15) within one minute in one **supergroup**, they are muted for `FLOOD_MUTE_DURATION_MS` (currently 3 minutes).
  ...
  Before allocating a candidate, the main-thread half (`packages/antiRaid/floodControl.ts`) checks the per-chat switch, supergroup type, whether the speaker is a real user, and whether that sender has the flood-control bypass. Channel identities and anonymous administrators have no member identity to mute — `restrictChatMember` only accepts real users, and Telegram does not reveal who is behind the disguise. `SUPER_ADMIN_USER_ID` always bypasses; each allowlisted identity follows its own `isCanBypassFloodControl`, which defaults to `true`, and participates in counting only when that permission is explicitly `false`.
