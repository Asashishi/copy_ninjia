...
- `config/whitelist.json` and `config/blocklist.json` are not optional files covered by the previous rule. The former controls synchronous authorization and insider protection; the latter is a static enforcement boundary.
  ...
  Both load strictly before network access or Worker creation, and a missing file, unknown field, or invalid ID aborts startup. `/white` and `/permission` atomically rewrite the complete allowlist only when it actually changes, publishing the new main-thread cache only after persistence succeeds; reads consult that in-memory copy. The static blocklist stays read-only and is unioned in memory with the dynamic `memory/blocklist/blocklist.json` layer; `/unblock` cannot remove a static entry.
...
- Generic JSON API requests may reach only HTTPS origins explicitly listed in `JSON_API_ALLOWED_ORIGINS`, with redirects disabled. A new caller must explicitly extend that allowlist. Telegram avatar crawling uses a separate Telegram-owned asset-domain suffix allowlist, while reusing the same HTTPS/no-credentials/label-boundary URL policy and disabling redirects as well. It must neither be routed through the JSON allowlist nor regress to accepting arbitrary HTTPS images.
...
- `/switch_mood` uses a main-thread request/waiter handshake with an AI Worker acknowledgement. The main thread must register the waiter before dispatch and settle it uniformly on timeout, Worker crash, abandoned restart, or shutdown. Each request carries an absolute deadline; the AI Worker must reject expired queued requests before the reroll side effect. Only a `moodSwitched` acknowledgement proves the reroll succeeded.

  If the later Telegram confirmation message fails, that failure must not be rewritten as “reroll failed.”
- AI-chat invalidation is an awaitable cancellation boundary. The first generation-sensitive task for a chat receives an epoch that is never reused within that Worker isolate. Invalidation synchronously deletes the current epoch, aborts the old generation, and clears work that has not started, then waits for the registered reply rounds, rate-limit notices, media descriptions, and memory compaction tasks under that epoch to settle before acknowledging `chatInvalidated`.
  ...
  **That wait must be bounded** (`AI_CHAT_INVALIDATE_DRAIN_TIMEOUT_MS`, comfortably below the main thread's `AI_CHAT_INVALIDATE_TIMEOUT_MS`): not every registered task honors the abort — memory compaction and media description cannot take an `AbortSignal` (`requestGeminiResponse` accepts none), and their retry delays plus request timeouts can run for minutes.
...
- **Two gates before acting, neither optional.** Read the bot's own permission bits first (next bullet), then confirm through the admin cache the join guard already keeps warm (`freshAdminIds`, falling back to `fetchAdminIds` when cold) that the target is not a chat administrator, and **never act when that cannot be confirmed**.
  ...
  When shutdown lands inside that queueing window the drain never settles, so the lifecycle refuses to confirm the Telegram offset and exits non-zero — after the restart the whole batch of updates is replayed (duplicate verification kicks and notices) and systemd reports a failed unit.
...
- A bare **chat** id (the negative id of a channel or group, `CHAT_ID_ARG_PATTERN`) is a separate switch and is **opened by `/unblock` only** (`acceptChatId`).
  ...
  **Every negative id carries `isChannel`** (stamped onto the minimal identity by `resolveIdTarget`, the same sign-based dispatch as `workers/antiRaid/blocklistEffects.ts`): `/unblock ... all` uses it to choose `unbanChatSenderChat` over `unbanChatMemberIfBanned`, and missing it makes the unban fail into `failedCount`, turning the reply into a false report about a target that was never touched.
...
- The authoritative `/block` list lives at `memory/blocklist/blocklist.json`; sibling `removals.json` is only an outbox of unfinished per-chat actions, never a list copy. The blocklist is a synchronous security boundary. The write path must update the main-thread in-memory Map (`packages/cache/main/blocklist.ts`) *before* posting the persistence message; the other order leaves a window in which an arriving join update reads a blocklist that has not recorded the ban yet, and that person gets in.
  ...
  **Cross-chat unbanning must go through `unbanChatMemberIfBanned` (with `only_if_banned: true`)**: the Bot API's `unbanChatMember` removes a user who is *currently a member* from the chat — that is exactly how `kickChatMember` implements "kick without ban" — so batch-unbanning without the flag would eject everyone who was sitting in those chats perfectly fine. Channel identities have no membership concept and use `unbanChatSenderChat`, which carries no such trap.
  ...
  With `all`, the unban runs even when the target was not on the list: "let them fully back in" is the point of the flag, and absence from the list does not mean absence of chat bans.
  ...
  **Insiders cannot be blocklisted**: `SUPER_ADMIN_USER_ID` and `config/whitelist.json` are rejected at the `/block` entry point — the list is irreversible, so replying to the wrong message would lock one of your own people out of every monitored chat forever. During startup recovery a single malformed record refuses startup outright: dropping one entry means letting that person rejoin.
  ...
  **A failure to register the join-time removal (outbox full, id space exhausted) must degrade in place; the exception must never escape `claimBlockedJoiner`**: it runs inside update middleware, so throwing fails the whole update batch, holds back the offset, exits non-zero, and systemd restarts into the same update — a restart loop that can only be broken by hand-editing `memory/blocklist/removals.json`, and a full outbox is usually the work of batches that can never be banned in the first place.
...
- `/ad_detect` ad detection is a **best-effort heuristic**, not a security boundary — but its disposal carries the exact weight of `/block`, so the boundaries must be drawn precisely.
  ...
  Submission is gated on a conjunction: the chat has `ChatState.isAdDetectEnabled === true`, the bot is an administrator there (the same `isBotAdminIn` decision the join guard makes — without admin rights the ad cannot be deleted nor the sender banned, so a verdict would just burn quota), and the sender is not one of our own (`SUPER_ADMIN_USER_ID` and `config/whitelist.json` are rejected at both the submission and the disposal entry; the blocklist is irreversible, so our own people must never even reach a verdict).
  ...
  **Conversely, Worker-side verdict batches must never be registered in the Anti-Raid in-flight task set**: that set is what the shutdown drain waits on, with a budget in the `ANTI_RAID_BARRIER_TIMEOUT_MS` range of seconds, while one verdict request can run to `DEEPSEEK_REQUEST_TIMEOUT_MS` (30s) times the empty-body retry.
  ...
  Registering them means that any shutdown coinciding with an in-flight verdict times the drain out, so the lifecycle refuses to confirm the Telegram offset and exits non-zero — a best-effort heuristic buying a dirty exit plus a batch of redelivered updates. When the drain arrives the pipeline only quiesces its ticker (no new requests, no further deletions or announcements) and lets the in-flight verdict finish on its own.
...
- A Telegram update may advance the acknowledgement boundary only after its middleware completes. Anti-Raid mailboxes, reaction/avatar background owners, and StateStore, AI Worker, and Disk I/O Worker flushes all have explicit bounded drains. Any critical flush failure must report failure, block final-offset acknowledgement, and exit nonzero.
  ...
  **The batch abandoned at shutdown counts too**: once the stop signal arrives the fetch loop no longer waits for the in-flight batch (its middleware may hang; draining is left to the lifecycle's bounded `size()` loop), so a failed update inside that batch can only be expressed by an explicit runner flag — written in the same synchronous section where `handleUpdate` throws, so it is already in effect when `size()` reaches zero.
...
- Updates within one batch may still run concurrently, but once the first `handleUpdate` rejects after its error handler completes, the runner must immediately abort the batch's other active updates and propagate that original first failure; it must not wait for a permanently hung sibling before stopping fetches. A separate `allSettled` observer continues consuming every late result to prevent unhandled rejections.

  A sibling that ignores cancellation remains visible through `size()` for the lifecycle's existing bounded drain but cannot delay first-failure propagation. No later batch may be fetched and no offset may advance after failure.
