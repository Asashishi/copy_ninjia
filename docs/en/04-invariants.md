...
- `config/whitelist.json` and `config/blocklist.json` are not optional files covered by the previous rule. The former controls synchronous authorization and insider protection; the latter is a static enforcement boundary.
  ...
  Both load strictly before network access or Worker creation, and a missing file, unknown field, or invalid ID aborts startup. `/white` and `/permission` atomically rewrite the complete allowlist only when it actually changes, publishing the new main-thread cache only after persistence succeeds; reads consult that in-memory copy. Startup also records a SHA-256 of the allowlist bytes, and every command-side full rewrite verifies those bytes first. An external edit or unreadable file rejects the mutation rather than silently erasing the edit with a stale cache. **That rejection, and any write failure, must be absorbed by the command itself and never escape the update handler**: `bot.catch` rethrows by design, the acknowledged runner then withholds the offset and exits non-zero, and Telegram replays the same command straight into the same throw — with a non-writable `config/` (EACCES/ENOSPC) a single `/permission` welds the bot into a permanent restart loop across every chat, and no number of replays can heal that cause. Nothing has been modified at that point (`commitWhitelistMutation` publishes the runtime snapshot only after the write succeeds), so acknowledging the update is safe; the reply must state plainly that nothing reached the disk, the same degradation `claimBlockedJoiner` uses. The static blocklist stays read-only and is unioned in memory with the dynamic `memory/blocklist/blocklist.json` layer; `/unblock` cannot remove a static entry.

  **The super administrator's permissions come from the identity itself, not from this file.** `getEffectiveWhitelistPermissions` in `packages/config/whitelist.ts` is the single source: `SUPER_ADMIN_USER_ID` resolves to `SUPER_ADMIN_WHITELIST_PERMISSIONS` (every key true), every other identity to the in-memory snapshot. `isWhitelisted` and `hasWhitelistPermission` both route through it, so callers only ever ask whether an identity holds a permission and never test for the super administrator per command. The override happens **on the read side only and is never persisted**: the write path `prepareWhitelistMutation` takes the raw configuration map, so a full rewrite by `/white` or `/permission` only ever emits entries the operator configured. Changing `SUPER_ADMIN_USER_ID` therefore cannot leave a stale all-true identity behind in the file, and a leftover entry under the same id cannot alter the effective permissions either. Both commands refuse the super administrator as a target at the entry point — `/white disable` excepted, so such a leftover can still be cleared — because anything else would only write a record that is never read back. **That exception must also change what it says**: it clears the leftover file entry and nothing else, since the super administrator's allowlist membership and all-true permissions are unaffected; reusing "removed from the allowlist" is a claim the very next `/permission query` disproves.

  **Both commands must also refuse the current chat's own identity as a target** (`sender_chat.id === chat.id`, the same reasoning as `/block` and `/unblock`): when an anonymous administrator posts under the group, Telegram exposes only that group's identity and the shared resolver returns it by design. Writing it into the allowlist makes every message sent under that group's anonymous identity bypass ad detection and the permanent blocklist (`isProtectedSender`), and a follow-up `/permission <group id> all` then hands `/block`, `/mute`, and the feature toggles to any anonymous administrator of that group — while Telegram never reveals who is behind the mask.

  `/permission query` and `/permission help` are read-only entry points. They read the effective permissions above under the command-sender identity; `query` returns only that identity's complete permissions after defaults have been applied (the super administrator receives the all-true view), accepts no target, and performs no write. Every permission mutation remains super-administrator-only. In groups, `help` remains permanently while `query`, refusals, and usage hints use the shared 30-second cleanup.
...
- AI replies count only successful text, sticker, reaction, and image actions against one unified action budget: the model-visible prompt limit is 8 and the execution hard cap is 11. Sticker, reaction, and generated-image actions may each succeed at most once; other action tools have no per-tool call cap. Sticker-pack viewing and Google Search retain independent lookup caps, while all custom function calls also share a whole-round loop guard.
  ...
  **Visible text has exactly two outlets**: a standalone message goes through `send_message`, while the line accompanying the image this round's `generate_image` produced goes through that tool's `caption`. A captioned generation is **one** Telegram message with one `message_id`, so it counts as one action and its self-record must be merged into a single entry (splitting it would make the same `message_id` appear twice in the transcript, leaving reply-chain traversal unable to tell which entry it refers to). When a caption exceeds `TELEGRAM_CAPTION_MAX_CHARS` the Bot API rejects the whole send rather than truncating, so the execution side degrades to "photo without caption + one standalone text message" and settles it as `actions_used: 2`.
...
- A human member's join verification accepts only that member's own click. The Worker must derive self-ownership from trusted `callback_query.from.id === callback_data` target ID rather than accepting a caller-supplied claim. Even a clicker inside the allowlist boundary (an entry in `config/whitelist.json`, or `SUPER_ADMIN_USER_ID`, which is always inside it) cannot verify another human; the sole vouching exception requires the current pending snapshot to have `isBot === true` and the clicker to be inside that boundary.

  A missing, terminal, or mismatched target may only receive a failure answer and must not mutate verification state.
- Before a terminal timeout/flood removal calls `kickChatMember`, it must query `probeChatMembership`: kick only after confirmed presence, settle silently after confirmed absence, and perform no destructive member action after an inconclusive lookup while retaining the terminal for its existing retry backoff. A `kickMember` emitted synchronously by a fresh join update during lockdown already has presence proof from that update and does not pay for a duplicate lookup **on its first attempt** — every joiner in a raid takes that path, so paying for one would double the call volume there. **Backoff retries must probe, however**: the join update only proves presence at that moment, and "kick without banning" in a supergroup maps to `unbanChatMember` without `only_if_banned`, so a `/block` ban placed during the backoff would be lifted by that very call while the outcome still reports `kicked` and the main thread has long recorded the removal as complete — the user can rejoin through any invite link. `kickPending` is never persisted, so its retry counter lives and dies with the state and is therefore a reliable "is this a retry" signal.
...
- The authoritative `/block` list lives at `memory/blocklist/blocklist.json`; sibling `removals.json` is only an outbox of unfinished per-chat actions, never a list copy. The blocklist is a synchronous security boundary. The write path must update the main-thread in-memory Map (`packages/cache/main/blocklist.ts`) *before* posting the persistence message; the other order leaves a window in which an arriving join update reads a blocklist that has not recorded the ban yet, and that person gets in.
  ...
  **`/unblock` performs a full unblock by default**: remove the target from the dynamic list when present, then lift the Telegram ban in every chat whose `ChatState.botIsAdmin` is true. The cross-chat unban still runs when the target is absent from the dynamic list. The command requires only `isCanUnBlock` (`SUPER_ADMIN_USER_ID` always holds it, so no separate admission is needed); the former `all` argument is neither parsed nor kept as a compatibility alias. A target from static `config/blocklist.json` must fail closed before either the list or Telegram APIs are touched, because the command cannot rewrite deployment configuration and lifting only the chat bans would create contradictory state.
...
- Both normal and abnormal shutdown first quiesce title, reaction, avatar, and translation entry points and stop the runner, then perform bounded drains. Failures from the four quiesce calls must be caught independently: a throw from one must not skip the others, and that attempt's failure must block final-offset acknowledgement and instance-lock release. A later `wait()` or `dispose()` may retry all four idempotent entry points. **Quiescence must never be cached as done** — `init()` re-arms those same four owners, so a stop signal during startup that latched success would short-circuit every later quiesce, leaving the owners accepting work throughout shutdown while the result still reports clean.
