...
- **Another bot's messages are not translated or copied, or only sometimes**: enable Bot-to-Bot Communication Mode (see [BotFather Settings](README.md#botfather-setup)). Translation handles only text; photos and captions are not translated, and a message containing a renderable `/command` is skipped entirely.
