...
- The **Disk I/O Worker** exclusively serializes reads and writes to `database/storage.sqlite`, `logs/`, and seven domains under `memory/`: `ai/`, `stickers/`, `luck/`, `anti-raid/`, `ad-detected/`, `joinlog/`, and `wed/`. The main thread writes `state.json` atomically through the business facade and `StateStore`. See [07 Data Root](07-operations.md#data-root) for every persistence shape and its recovery and retention role.
...
Each chat processes at most `REPLY_ROUND_MAX_CONCURRENT` (currently 5) AI requests concurrently. Direct triggers that have not started remain in the FIFO capped at `REPLY_TRIGGER_QUEUE_MAX` (currently 15) and leave it when their round starts. Each round reserves its delivery position in ingress order, before asynchronous media recognition. Delivery uses 5 fixed buckets, each able to append multiple chains; delivery backlog does not count toward model admission. When a model finishes, it commits the complete action chain, immediately releases its model slot, and starts more waiting work. Only the earliest ready round sends; pending placeholders wait and completed entries are skipped. Actions within a round follow tool-call order, and follow-up actions await actual results. The round task retains delivery, heartbeat, and sticker-lock ownership until actual cleanup, then reclaims the delivery position. Trigger rate limits, provider quotas, Telegram throttling, 429 retries, and cancellation retain their existing boundaries.
