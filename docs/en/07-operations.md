...
- `Cron task "<name>" action #<n> (<type>) failed in chat <id> after <k> attempt(s)`: a `chat_id: "all"` task finally failed in one group; only that group's remaining actions were skipped and the other groups still received the run. Read the cause as above. `Cron task "<name>" skipped <n> chat(s) without send permission.` is an ordinary log line: some groups were skipped this run because the bot lacked a send permission there or the lookup failed.
