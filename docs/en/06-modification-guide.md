...
4. Write the cold-migration script and **replace** the single edge declared in `scripts/conventions/coldMigrations.ts` — the convention counts migrations rather than releases and allows exactly one "format produced by the previous migration → format produced by this migration" edge, so the old script and its tests are deleted with it.
