...
The fifth key, `randomImageDir`, is the directory `/h_image` and cron `rand_image` draw pictures from (see [08 Commands](08-commands.md)); it defaults to `images`. A relative path resolves against the runtime data root (the directory holding `state.json`); an absolute path is also accepted. A missing directory is created at startup; a path that exists but is not a directory, or that cannot be created, refuses startup and names the field path. Adding or removing pictures needs no restart, since the directory is listed afresh on every use. Only `jpg`, `jpeg`, `png`, and `webp` files directly inside the directory count; hidden files, subdirectories, and symbolic links do not, and a picture over 10 MB is reported instead of sent. On the first run there is no `state.json` in the data root yet; it is created by this seeding once startup succeeds.
