...
- 主进程先递归创建并预检运行时数据根的写入、文件 fsync、hard link、原子 rename 与目录 fsync，再取得 `bot.lock`；数据根及敏感顶层 `memory/`、`logs/`、`database/` 必须是实际目录，`lstat` 命中符号链接即 fail closed。显式配置 `COPY_NINJIA_DATA_ROOT` 时，数据根、`memory/` 与 `logs/` 要求 mode 不宽于 `0755`，即禁止 group 与 other 的写位；`database/` 为 SQLite 旁路文件保留 `0770` 协作组写入上限，非运行 UID 所有时还必须属于运行进程的有效组且组位为 `rwx`。已有目录只校验、不自动 chmod。随后清理顶层孤儿临时文件并严格恢复 `state.json`，这些步骤发生在任何联网和 Worker 创建之前。`bot.lock` 的 guard 与 recovery 辅助文件都用 hard link 原子发布，但回收陈旧副本是「判死 → 删除」两步、两步之间不原子：同一数据根下两个进程同时启动、且现场同时残留两份陈旧辅助文件时，两者可能都认为自己持有 recovery。线上由 systemd 单实例拉起，同一数据根不会有第二个启动中的进程，这条路径不可达；协议按现状保留，不为它加分布式锁（见 `infra/storage/instanceLock.ts`）。

  部署输入严格校验通过后，初始化 Disk I/O Worker 并恢复持久化数据；恢复成功后才初始化 Telegram 客户端、完成 handler/命令菜单/`bot.init()` 握手，随后初始化并 hydrate AI/Anti-Raid Worker，启动 acknowledgement-safe runner。
...
- **进程级 Telegram 身份严格来自 `config/bot.json`**：`bot_token` 与 `super_admin_user_id` 联网前必检，缺失、未知字段或非法值均拒绝启动。AI key 全部属于 `config/agent.json` 中的能力配置；每项能力独立声明 provider、api_key、base_url 与 model，不存在凭据默认、跨能力回退或运行时覆盖。`base_url` 只接受 `https`，明文 `http` 仅限 `localhost`/`127.0.0.1`/`::1`，且不得带 userinfo 或 `#` 片段——它旁边就是同一项能力的 api_key。
...
- **启动总闸只校验「已经存在」的部署输入，缺省与否交给功能 readiness。**启动总闸调用 `packages/config/readiness.ts` 的 `validateExistingDeploymentInputs`：`bot.json` 是进程级必填，其余可选输入（`stickers.json`、`mood.json`、`ad_samples.json`、`agent.json`、`g-auth.json`、`prompt/persona.md`）**只要文件在就必须严格解析通过**，非法内容不因对应功能当前关着而被掩盖；文件真正不存在时不阻止启动。
...
- **复读与翻译只经一个出口**（`copy/echo.ts` 的 `sendEchoPayload`）：文字与图注按字符串处理后交来；纯文字 `sendMessage`（不带实体、不设 parse_mode，照搬原消息的 `link_preview_options`），付费媒体只发文字，其余消息一律 `copyMessage`，有文字时经 `caption` 换成新图注并照传 `show_caption_above_media` 与视频 `start_timestamp`，文件、缩略图、剧透与封面由 Telegram 服务端复制，本机不下载。长度上限（正文 `TELEGRAM_MESSAGE_MAX_CHARS`、图注 `TELEGRAM_CAPTION_MAX_CHARS`）由调用方判定，超限整条丢弃。`check:conventions` 禁止在 `sendEchoPayload` 之外用 `copyMessage` 替换图注。
...
- **一轮私密模式到期后必须推进恢复**。`LOCKDOWN_MS` 的截止时间在进入 `ACTIVE` 时确定，再次入群不重排计时。到期先持久化恢复意图，durable ACK 后调用恢复 API；`RESTORING` 收到阈值事件只预热管理员缓存，不回到 ACTIVE。恢复失败按现有重试处理，外部 API 成功时间不作瞬时保证。成功后清空本群入群滑窗，公告删除失败只记日志，下一轮从后续入群重新计数。
...
- AI chat invalidate 是可等待的取消边界：每个群首次接纳 generation-sensitive 工作时取得本 Worker isolate 内永不复用的唯一 epoch；invalidate 同步删除当前 epoch、abort 旧代并清空未开始任务，再等待该 epoch 下已登记的回复轮、限频提示、媒体描述与记忆压缩 settle，最后按 request ID 回 `chatInvalidated`。
  ...
  **这个等待必须有上限**（`AI_CHAT_INVALIDATE_DRAIN_TIMEOUT_MS`，且明显小于主线程那道 `AI_CHAT_INVALIDATE_TIMEOUT_MS`）。记忆压缩与媒体描述均向模型请求传递本代 `AbortSignal`，压缩重试等待也接收该 signal；外部请求收到取消后仍可能不能及时 settle。用于 `Promise.race` 的 unref 到期 timer 无论任务先完成还是超时先到，都必须在 `finally` 清除。超时只结束等待，旧回复轮仍计入存活容量，直到实际收尾。
  ...
  主线程必须同时等该回执与记忆删除 durable 才能宣称 `/ai_chat disable` 或 `/clear_context` 完成——两条命令共用同一条 `invalidateAiChat(chatId)`，都要求本群记忆清空并将 `chat_states.ai_context` 置为 NULL，保留 `ai_persona`，区别只是前者还落一次开关。Worker 崩溃、放弃重建、投递失败、超时或停机都必须 reject waiter。

- `/clear_context` 只按发起身份的 `isCanClearContext` 清除当前群对话记忆，保留自定义人设。超级管理员始终直授 true；新白名单成员默认 false。授权与撤销由 `/permission` 独立控制。
...
- 回复行动工具先同步校验并返回 `success: true, queued: true, actions_used`，模型按调用 ID 收到的是接纳回执，不含真实消息编号；校验失败仍立即返回错误。每次接纳建立独立调用链，生成、拟人停顿、Telegram 排队及其重试都在链内执行。同轮动作按工具调用顺序串联，链内依赖真实返回值继续执行补发和自录；整轮发送按入站顺序执行，后一轮不能插入前一轮的正文、纠正或图注之间。失败由链统一收尾并记录，模型不得凭乐观回执重投。
  ...
  `activeReplyCounts` 只统计模型未完成轮次，同群上限 `REPLY_ROUND_MAX_CONCURRENT` 为 5，Telegram 高压时为 1。`pendingReplyTriggers` 每群保留最多 `REPLY_TRIGGER_QUEUE_MAX`（15）个未启动的直接触发；25 个受管群的总上界为 375，群 teardown 清理等待项。`replyDelivery.ts` 在 AI Worker 内按入站顺序预留发送位置，5 个固定桶中的 FIFO 共同受跨代际存活预算限制：单群 `REPLY_DELIVERY_MAX_PER_CHAT`（32），Worker 总计 `REPLY_DELIVERY_MAX_TOTAL`（128）。容量满时随机触发丢弃、直接触发排队，队满登记临时溢出提示；reserve 失败不扣限频额度、不创建提示词或模型任务。启动成功才移除队首，媒体解析在占位后进行。模型结束时 commit 完整链并释放模型位，发送只放行就绪队首，空轮与失败项按序跳过。实际发送、心跳及贴纸锁收尾后才释放存活容量；`onFinished` 处理溢出提示并推动等待群补跑，取得全局空位且仍有队列的群移至末尾。invalidate、reset 和失效等待超时只撤销当前窗口，不提前归还旧任务容量；旧代收尾不得删新窗口，Worker 销毁后计数自然释放。五分钟限频、触发 FIFO、供应商 lane、Telegram throttler 与分类 429 队列继续按各自边界生效。轮次仍登记在原 generation 任务集中，等待所有调用链及其心跳收尾、释放贴纸锁并回收发送顺位后才移除；模型完成或发送收尾通知异常均不得跳过清理。失效和停机取消信号继续传到真实请求与重试队列。记忆容量淘汰优先跳过仍有模型或当前代际任务的群，全部候选活跃时仍按 LRU 淘汰。只有实际发送成功的消息和 Telegram 返回的回复关系进入自录；接纳回执不写记忆。
...
- 主线程的 AI 活跃度概率层是随机主动搭话的准入闸门，不是 AI 回复轮次的限频器：每条可见群消息先记入本群滑动窗口，近期消息越多，随机触发概率越高，但到达热群上限后不再提高；不同群互不借用热度，进程重启则从冷群状态重新开始。直接触发不受这道概率闸影响。常量统一由 `packages/consts/aiChat/rateLimit.ts` 管理，文档只约束这些语义，不固化可调参数。
  ...
  随机回复的「群 × 发言人」冷却由主线程 `userReplyTriggerTimes` 保存，使用单个 unref 清扫 timer。满表扫描只记录当时全部条目有效的区间 `[validFrom, validUntil)`；区间内的新键直接拒绝，边界到期或时钟回拨至区间之前时重新扫描。满表中的旧键换代和清空会失效区间；容量与冷却时间由生产常量控制，有效项不淘汰。
...
- **动手前两道闸缺一不可**：先看机器人自己的权限位（下一条），再用入群守卫本来就热的管理员缓存（`freshAdminIds`，冷了才 `fetchAdminIds`）确证目标不是本群管理员，**确证不了一律不动手**。这个三态判定（`true`=是管理员／`false`=确认不是／`undefined`=没查出来）是一条权限边界，只能有一份实现，和两个取数函数一起住在 `workers/antiRaid/adminCache.ts` 的 `isChatAdmin`；刷屏禁言与广告处置各抄一份的话，哪天改了兜底语义（比如把 403 当成「不是管理员」）只会改到一处，两条链路从此对「谁豁免」各执一词。广告那侧的「频道马甲直接判 false」是它独有的前置，留在调用点，不进这个共享判定。权限位那道闸是三态的，**「没观测到」不当成「观测到没有」**：确证没有才就地放弃并保留抑制位；没观测到则照常往下走、由 Telegram 的回应当裁判——镜像可能只是还没到（主线程的按需现查撞上一次 429 就会退避几分钟），那几分钟里把刷屏放过去、还在日志里写一句没有依据的「没有权限」，比多打一个注定失败的请求糟得多。
  ...
  群内通知仅在禁言成功后发送，由主线程在发送成功的 `onSent` 边界登记 30 秒删除任务。timer 与停机 `flushPendingMessageDeletions` 共用删除 owner 和在途集合，drain 等待已经开始的删除。通知设置 `batchOnFlush`，停机按客户端和群分组，以每批最多 100 条调用 `deleteMessages`。
...
- 候选信源只有 `memory/wed` 的成员 ID 集合：抽中后只按 ID 调 `readCurrentAvatar`，身份（图注和公开头像兜底都要用）取自它本来就要发的那次 `getChat` 返回的私聊资料，不发 `getChatMember`，因此抽取不要求机器人是群管理员，也不多花请求；`getChat` 核实出来不是私聊用户的候选按「没有可用头像」计入配额。不判断是否仍在群、不排除机器人，也不回写集合——机器人由写入侧挡在集合外，离群成员由退群更新和每日复核清理，整群停管由 teardown 删除。`readCurrentAvatar` 返回三态：只有「确认没有可用头像」消耗每轮 8 个候选的配额；头像查询未完成不计入配额，累计 3 次即放弃本轮。更换排除当前目标；图注中的两位用户均使用提及实体。候选查询、头像下载及图片发送/编辑共用 30 秒请求预算，同时服从 update 取消和会话 teardown。图片字节不进入会话，不写文件或数据库；头像下载复用 Telegram 下载闸、有界读取和公开资产域校验，不调用头像设置 API。
...
- `commands/wed/chats.ts` 集中管理交互创建与会话清理。群 teardown 同步摘除交互状态、撤销本群排队项并取消全部会话，再依次删除清理开始时的空闲结果。长期成员集合的去留只看 teardown 起因：`lostAuthority` 保留，`explicitDisable` 与 `departed` 经 `purgeWedMembers` 连同 `memory/wed/<chatId>.json` 一起删掉。**成员集合的删除不挂在「交互表里还有没有这个群」上**——本进程从未交互过的群同样要把奖池删干净。忙碌项的迟到结果由自身 `finally` 清理，不会因为清理等待期间变为空闲而再次删除；teardown 后的旧按钮失效，重新创建的同群交互复用成员集合。普通移除失败保留会话供重试，Telegram 删除失败走统一错误边界。进程停机先停止接纳并排空已接纳任务，再投递脏成员快照，最后由 DiskIO flush 确认写入；任务超时或投递失败阻止最终 offset 确认和实例锁释放。该 owner 在统一延迟删除和 Telegram 出站之前 drain。Worker 崩溃不清空主线程集合，进程重启恢复成员但不恢复会话，旧按钮只提示重新发送命令。
...
- 随机图片目录来自已校验的 `state.global.assets.randomHImageDir`（缺省 `./h_image`，只收绝对路径或 `./`、`../` 开头的显式相对路径，相对路径按运行时数据根解析，`infra/storage/stateStore.ts` 的 `getRandomHImageDirectory`）。启动时在部署输入闸之后、任何 Worker 与外部连接之前由 `infra/randomImage.ts` 的 `ensureRandomImageDirectory` 准备：是目录（含指向目录的符号链接）则不动，缺失则创建，存在但不是目录或建不出来就拒绝启动，诊断只写 `state.json` 路径、字段路径与解析后的目录。运行期目录被删不自动重建。
...
- 执行（`packages/cron/run.ts`）：动作按顺序、间隔 1 秒；网络错误、5xx、出站闸重试后仍返回的 429 与出站队列满按 2/4/8 秒退避最多重试 3 次，`send_voice` 的合成失败、等待超时与 AI Worker 暂不可用同样可重试，`tts` 缺席与编码失败不重试，其余失败不重试；每轮新建一张 `CronRoundVoices`，同一轮里一个 `send_voice` 动作只合成一次，重试与后续会话复用同一段语音，首次发送成功后记下 Telegram 交回的 `file_id`，之后改为引用它、不再重复上传，轮次结束即丢弃；最终失败记一条英文错误日志并中止本轮剩余动作。重试可能在超时误判时重复发送。
...
- `packages/cron/delivery.ts` 是唯一发送边界（`check:conventions` 禁止 `packages/cron/` 其它文件发送），经主线程 grammY 客户端走 throttler 与 429 分类闸，不设 `parse_mode`，不带论坛话题（开了话题的群里落在 General），成功后登记自发消息。cron 消息是用户授权的长期保留例外，不挂固定延迟删除。`url` 原样交给 Telegram 拉取，本机不下载；本地文件先按 Telegram 上传上限（图片 10 MB、文件 50 MB）判超限，上传源是每次序列化都重新打开的 `Bun.file().stream()`，429 重放不会拿到读完的流；`rand_image` 复用 `infra/randomImage.ts`，每次尝试重新抽取；`send_image` 的 `is_blurred` 为 `true` 时请求带 `has_spoiler: true`，缺省时请求里不出现该字段；`send_voice` 取本轮语音（没有时经 `synthesizeVoice` 合成并登记），本轮还没有 `file_id` 时以 `voice.ogg` 上传、有了就直接引用 `file_id`，调用 `sendVoice` 并带上整秒时长，不写 AI 记忆。
...
- 统一 logger 在写入 journal、Worker 信封与 `logs/` **之前**同时执行两层脱敏：已加载配置中的凭据按值替换；SDK/HTTP 错误对象中的 `authorization`、`cookie`、`set-cookie`、API key、token、secret 与 password 等凭据字段按键替换，原始 header tuple 形态同样覆盖。后一层不能只靠 配置凭据清单——xAI/Cloudflare 响应 Cookie 不是本进程配置值。实现必须复用既有 JSON 序列化遍历，不得为每条错误日志深拷贝对象；request id、限流余量与 token 数量等非凭据诊断必须保留。
...
- **群通知按 Bot 配置选风格，自定义人设优先普通语气**：两套只读文案存于 `packages/consts/atmosphere/{teasing,plain}/`。机器人所在群设置了 `aiPersona` 优先用普通版；无人设时按 `bot.json` 的 `atmosphere` 选择（`mesugaki` 雌小鬼，`normal` 普通，缺省雌小鬼），与 `isAIChatEnabled` 无关。主线程直接读现有群缓存，AI Worker 读现有人设镜像；Anti-Raid 的 `cache/workers/antiRaid/atmosphere.ts` 只保存有自定义人设的群 ID，容量受 25 群状态上限约束。主线程在配置或删除落盘后增量推送，Worker 初始化和崩溃重建时在业务接管前全量重放；缺项即初始化载荷指定的 Bot 配置语气，停机清空，被撤管理员保留。每条群消息不增加 SQL、镜像同步或 request/reply。

- **通知的渲染阶段只复用局部文案引用**：同条消息的正文、称呼与按钮复用当前文案表；配置变更完成后的回执、下一次看板操作和后台发送重新选择。头像队列只保存目标、命令来源与代次，头像操作完成且任务仍为最新代次时，才按结果和当前群人设生成回执；被覆盖或取消的任务不发送回执。广告候选直接使用消息上下文已有的 `chatState.aiPersona`，不再次查询群缓存。主线程不另建群风格缓存；两个 Worker 只保存经初始化消息注入的 Bot 缺省风格枚举，重建由 main 重放本进程启动快照。

- **菜单与通知共用同一风格选择**：启动按 Bot 风格注册所有群聊作用域菜单，再恢复各群作用域；`/prompt config` 设置普通版群菜单，`/prompt remove` 或群状态清除删除群作用域，`/init enable` 重新核对本群菜单。菜单 API 失败只记录日志，后续启动或人设操作重新同步。权限 help/query、QA、按钮、命令回执和 Worker 通知使用所选文案，保留既有留存边界。验证提醒每次重试重新选择正文与按钮；AI 限频提示的自录使用实际发送文本。用户内容只插值，实体偏移在渲染后计算。
...
- **每群状态的权威副本是 SQLite `chat_states` 表；主线程只持有一份容量恰好等于 `STATE_MANAGED_CHAT_LIMIT`（25）的热读副本**（`packages/cache/main/chatState.ts`）。状态字段保存在 `status`，覆盖七个功能开关（含 `isProxySendEnabled`）、`quietUntil`、`lockdown` write-ahead 记录、`botPermissions` 完整快照、`title` 与翻译会话 `translate`；`aiPersona` 从独立的 `ai_persona` 列合并到同一 `ChatState`。
...
- 黑名单移除批次必须跨进程存活：主线程在投递 Anti-Raid Worker 前，把当前 `pendingBlockedRemovals` 快照交给 Disk I/O Worker，并按独立的 `blocklistRemovalOutbox` 领域等待 SQLite `pending_blocked_removals` 变化的 snapshot revision ACK；只有对应 transaction durable 后才能交接 update。Worker 把新旧快照比较成按主键的 upsert/delete，只编码实际变化的行，不再为每次销账整份重写文件。
  ...
  **补扫条目（`probeMembership: true`）不得持久化 `userIds`**：outbox 只记录「拿当前黑名单扫这个群」的任务。投递与重放按主键升序从 Disk I/O 边界读取稳定游标页，每页最多 512 个 id；上一页收到完整回执后才读下一页，最后一页落定后才能销掉 durable task。每页读取前先确认 blocklist 领域的写入已经 flush/ACK（回执驱动的写入必须避开这段窗口，见[黑名单销号识别](#黑名单销号识别)），Disk I/O Worker 再把 SQLite 已提交页与最多 128 条并发 pending 最终值合并；不得把多页拼回完整 `Set` 或数组。任一页读取或投递失败都保留 outbox、释放本轮 claim 并推进退避，下次从空游标安全重放。把名单冻结进每个群任务会放大成「群数 × 名单长度」的存储和 structured-clone 成本，而且重放时已经过期。反过来，秒踢类 `probeMembership: false` 任务必须冻结当时已确定的非空 `userIds`；两种 shape 由判别联合与严格 codec 同时约束。
  ...
  4,096 条容量上限在 Disk I/O 启动 inspect 分页累计时、运行期接收新 revision 快照时分别校验；超限快照不分配待写表、不提交、不 ACK。`lastFailure` 首次转为 `missing-permission` 时立即排入下一份 durable 快照，权限闩锁可在重启后恢复。黑名单处置的用户间等待与请求重试等待均订阅 Worker 停机 signal，取消时以 `complete: false` 保留 outbox 重放。
...
- AI 记忆恢复只接受满足当前 `AI_MEMORY_HYDRATE_BUFFER_MAX` 与 `MAX_SUMMARY_ROUNDS`（当前为 255 条逐字消息与 7 轮冷摘要）的 version=1 快照；超限或字段非法都拒绝启动，不在恢复时截断。提高容量上限与已有快照兼容；降低上限时，必须先停止旧进程，再以同一严格 codec 在 SQLite 事务中重写现有 `chat_states.ai_context` 快照，避免旧进程的停机 flush 覆盖迁移结果。
  ...
  当前消息及引用中的姓名、用户名、转发来源、正文和引用文本必须是单行字符串，空白只允许普通空格；正文非空，引用 text/quote 最长 500 个 UTF-16 码元。截断产生的末尾空格或单个代理码元保留。`at` 必须是有效东京本地时间 `YYYY/MM/DD HH:mm:ss`。`pendingImage` 可缺省（缺省即内容态，写盘时不输出该键）；存在时必须恰好含 `origin`（`command` / `generated` / `referenceGenerated`）与单行 `caption` 两个字段。摘要和 pendingSummary 可以换行。Disk I/O 恢复与 AI Worker hydrate 共用严格 decoder，非法内容按具体字段路径拒绝，不清洗、不丢条、不回写。
