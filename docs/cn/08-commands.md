...
- **群内通知语气与菜单**：机器人在当前群且配置了自定义人设时，命令提示、权限 help、按钮和自动通知使用普通版；未配置或移除后恢复雌小鬼版。AI 开关不改变这项选择。`/prompt` 成功落盘后同步本群 Telegram 菜单，其他群不受影响。普通 inline 运势查询没有目标群 ID，仍使用默认雌小鬼版。菜单只注册在所有群聊作用域（`all_group_chats`），启动时清掉默认作用域，私聊里不显示菜单；私聊里的命令除超级管理员的 `/send` 外一律不回复。
...
候选来自每群独立、长期复用的 **`Set<number>`，最多 15 万人**。只记录已初始化群中以个人身份实际发言的用户，不从频道发言、回复、转发来源、频道自动转发或匿名群身份扩充候选。重复发言不改变集合；满额保留已有成员并暂停新增，退群腾出空间后继续记录。退群更新先删除 ID，群停用期间收到的退群事件也会清理已有记录。候选抽中后只用一次 `getChatMember` 取身份，不判断是否仍在群，也不回写集合；离群成员由退群更新和每日复核清理，Telegram 认不出 ID（`PARTICIPANT_ID_INVALID`）的已销号账号也在复核时移除。发起人不参与自己的抽取，每轮最多检查 8 个确认没有可用头像的候选；查询未完成不占配额，累计 3 次即放弃本轮。候选不足或头像不可用时提示稍后重抽。

头像先由 `getChat` 确认，再从 `getUserProfilePhotos` 返回的前 100 张头像中按 `big_file_unique_id` 匹配对应尺寸。匹配成功后，发送和更换都直接复用 `PhotoSize.file_id`，本机不下载或上传图片；未匹配或头像列表查询失败时，继续下载当前 ChatPhoto。必要时使用本轮查询返回的公开用户名，复用 `/icon steal` 的公开头像抓取能力。图注中的发起人和候选均使用用户提及实体。所有下载都有体积上限和取消边界；不会修改机器人自己的头像，也不消耗 copy 类命令的冷却。
...
- **来源**：每次现查——用户读本群的成员身份，频道或群读会话资料，机器人自己用启动时的自身资料；查不到时退回机器人记得的身份，什么都没有就回「查不到」。头像与 `/wed` 同一套读取（用户头像按 file_id 复用，频道头像下载后上传，另有 t.me 页面兜底），读不到就不带图。
...
- **回执**：不回复消息、被回复的消息里没有图、没有权限时只回一句提示；收完回一句汇总：新收几张、收图前图库里原有几张（口径同 `/h_image` 抽图的候选）；有图库里早已有而跳过的、或没收成的，再补上各自张数。所有提示都在 30 秒后删除。
