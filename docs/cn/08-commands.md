...
- **群内通知语气与菜单**：机器人在当前群且配置了自定义人设时，命令提示、权限 help、按钮和自动通知使用普通版；未配置或移除后恢复雌小鬼版。AI 开关不改变这项选择。`/prompt` 成功落盘后同步本群 Telegram 菜单，其他群不受影响。普通 inline 运势查询没有目标群 ID，仍使用默认雌小鬼版。
