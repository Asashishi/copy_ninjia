...
定时任务由部署方配置 `config/cron.json`，支持热重载，无群内编辑命令。可向指定会话、所有已启用且有发送权限的群，或排除部分群后的集合发送文字、图片与文件。支持时区、一次性任务和随机间隔；固定图片用 1–10 项 `url` 或文件 `path` 数组，2–10 张合为相册，仅首图带说明；随机模式每次一张，可指定独立目录。`is_blurred` 控制全部图片的剧透遮罩。
