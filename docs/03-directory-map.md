...
| `packages/antiRaid/` | Anti-Raid 主线程代理：Worker 监督入口、lockdown 恢复、待验证镜像接收、黑名单入群判定、广告判定的投递/处置与刷屏计数投递 | `index.ts`、`lockdownMirror.ts`、`verificationMirror.ts`、`blocklistGuard.ts`、`adDetect.ts`、`floodControl.ts`、`memberFacts.ts` |
...
`packages/ai/` 这类被多条线程复用的领域代码要留意：一个只被主线程用到的纯函数，若与 Worker 独占的缓存同住一个文件，主线程 import 它就会把那份缓存一并实例化。范例是 [`packages/ai/stickers/describe.ts`](../packages/ai/stickers/describe.ts)——它从 `sets.ts` 拆出来，就是为了让主线程的消息流水线拿到贴纸描述函数时不碰 AI Worker 的贴纸集合缓存。
