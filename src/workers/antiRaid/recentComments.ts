 ...
 * 回复降级）。
 */
export function rememberRecentComment(chatId: number, userId: number, messageId: number, repliesToChannelPost: boolean): void {
...
export function takeRecentComment(chatId: number, userId: number): { messageId: number; repliesToChannelPost: boolean } | undefined {
  const key: string = verificationKey(chatId, userId);
