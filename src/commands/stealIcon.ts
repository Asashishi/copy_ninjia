...
import type { CachedUser, GlobalCopyState, UsersFileSchema } from "../types";
import { saveChatUsersEntry, saveGlobalCopyState } from "../infra/storage";
...
export async function handleStealIconCommand(
  ...
  // users.json 里本群的 copiedUser 保持原样——偷头像不影响正在进行的复读
  // （没有复读时该字段本来就是 null）。
  await saveGlobalCopyState(globalCopyState);
  await saveChatUsersEntry(usersFileData, chatId, usersFileData[String(chatId)]?.copiedUser ?? null);
