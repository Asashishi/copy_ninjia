...
export const GENERATE_IMAGE_TOOL_INSTRUCTION: string =
  "根据群友当前请求生成一张全新的 1K 图片并直接发送到群里。调用的硬前提是：本轮触发消息直接回复或 @ 了你，且消息本身明确要求画图、生图、" +
  "做海报/壁纸/视觉稿，或明确要求把想法呈现成图片。仅仅提到图片、描述场景、讨论构图、询问你是否会生图，或你觉得配图更好，" +
  "都不构成调用意图；不得根据暗示或自行发挥擅自生图。prompt 必须是可独立交给图片模型的完整画面说明，" +
