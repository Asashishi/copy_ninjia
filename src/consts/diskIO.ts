 ...
 * 两类落盘共用同一个 Worker。目录路径见 paths.ts。
 */
...
// ---- 日志的追加窗口 ----
// 日志落盘是"按位置追加"（见 workers/diskIO/appendOnlyDayFile.ts）：entries
// 只增不改，条数不像 AI 记忆快照那样有固定上限，值得按条数或时间双阈值
// 分批落盘，不必每来一条就单独触发一次追加。内存 buffer 攒满
// FLUSH_MAX_ENTRIES 条、或距首条入队 FLUSH_INTERVAL_MS 时批量落盘。
 ...
 * 见 snapshotFiles.ts 模块头注释），不需要额外的条数阈值。
 */
