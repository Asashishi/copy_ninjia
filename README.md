...
- **AI 闲聊**：基于 DeepSeek API 和自定义人设（`prompt/persona.txt`），概率性地在群里生成闲聊回复；回复机器人或 @ 机器人时必回，纯按概率命中的随机搭话则不挂 Telegram 回复引用，改为在文字里点名称呼触发者。
  ...
  - 分群三重限频：0.5 秒冷却 + 每分钟最多 45 次 + 每 5 分钟最多 150 次（滑动窗口），超限的触发静默丢弃，防止恶意刷屏烧穿 API 配额。
...
- **`/quiet`**：让机器人在本群安静一段时间（`/quiet [分钟数]`，1~15，缺省 3 分钟），期间不触发 AI 随机搭话、随机复读等主动行为；回复/@ 机器人的必回和各类指令不受影响。
