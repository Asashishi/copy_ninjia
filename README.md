...
- 单群单飞：同一群只允许一轮 Gemini 工具对话在途，避免旧请求迟到后倒序发言。
- 限频：0.5 秒冷却、每分钟最多 45 次、每 5 分钟最多 150 次；超限提示本身也有冷却。
...
项目启用了 `strict`、`noUncheckedIndexedAccess`、`noUnusedLocals`、`noUnusedParameters` 等检查。新增共享协议放进 `src/types/`，调参值放进 `src/consts/`，运行时状态放进对应 `src/cache/`，避免业务文件继续长出游离状态。
