...
- **反刷群（Anti-Raid）**：采用滑动窗口算法检测刷群行为，最近 60 秒内入群人数超过 45 人时，自动临时开启私密模式（禁止普通成员拉人），到期后自动恢复原权限；私密模式期间的新入群直接踢出（评论区留言进来的人例外，照常走验证流程）。支持管理员/群主入群豁免（免验证且不计入刷群统计）。私密模式状态支持本地文件持久化（`lockdowns.json`），在进程重启后能自动接管并重排恢复计时，避免群权限被永久锁定。
...
- **多群独立状态**：每个群聊的复读目标、静默时间等状态互相独立；copy 类命令的全局冷却时钟跨所有群共用一份。两者都保存在同一个 `state.json` 里，重启后自动恢复。
