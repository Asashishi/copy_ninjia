...
- 触发：回复机器人或 `@机器人` 时必定触发；普通文字和媒体评价共用按群活跃度的动态概率：近 1 小时每观察到一条群消息，概率分母从 150 减 1，最低到 15；活跃度只存内存，空闲超时或重启后回到冷启动。
...
- 最低配置（文本为主、仅少量群开启 AI）：4 vCPU / 2 GB RAM / 本地 SSD；2 GB 不适合作为媒体洪峰下的内存保障
