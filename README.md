...
- **反刷群（Anti-Raid）**：采用滑动窗口算法检测刷群行为，最近 60 秒内入群人数超过 30 人时，自动临时开启私密模式（禁止普通成员拉人），到期后自动恢复原权限。支持管理员/群主入群豁免（免验证且不计入刷群统计）。私密模式状态支持本地文件持久化（`lockdowns.json`），在进程重启后能自动接管并重排恢复计时，避免群权限被永久锁定。
