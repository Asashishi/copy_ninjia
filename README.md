...
- **刷屏禁言**：同一个人在同一个超级群内一分钟发言达到 15 条，就地禁言 3 分钟并在群里说明一句（公告在禁言解除时自撤）。到点由 Telegram 自动解除，不写黑名单也不删消息。仅在机器人确有「限制成员」权限时触发；群主/管理员、`SUPER_ADMIN_USER_ID` 与 `config/whitelist.json` 白名单身份、频道马甲与匿名管理员都不计数。
