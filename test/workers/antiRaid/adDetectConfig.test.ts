...
describe("Anti-Raid Worker 接管广告检测配置", () => {
  test("新快照整体替换 holder，并丢弃旧客户端与 system prompt 缓存", () => {
    ...
    adoptAdDetectConfigMessage({ type: "agentConfig", adDetect: reloadedAdDetect, adSamples: samples });
  ...
  test("广告检测不可用时示例 holder 与提示词缓存保持不动，ad_detect 显式写 null", () => {
    ...
    adoptAdDetectConfigMessage({ type: "agentConfig", adDetect: null, adSamples: null });
