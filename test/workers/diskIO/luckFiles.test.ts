...
/** 把当日文件位置占成目录，让 BunFile.text 恒抛 EISDIR。 */
...
describe("diskIO/luckFiles：运势缓冲/落盘调度", () => {
  test("新 key 入缓冲并排定时器，flush 后写入文件、缓冲清空", async () => {
  ...
  test("同 key 不同值（restoreLuckState 丢弃旧记录后当天重派生）必须再次落盘，恢复时取最新值", async () => {
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "大吉", fortunePercent: 90.12 }));
    await flushLuckAppends();

    // 模拟 LUCK_TIERS 改动后重启：主线程为同 key 派生出按新档位表解释的结果
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "小凶", fortunePercent: 39.99 }));
    expect(luckPendingAppends.length).toBe(1);
    await flushLuckAppends();
  ...
  test("启动恢复遇到尾部截断时 fail closed，并保持原始字节不变", async () => {
  ...
  test("启动恢复遇到非法运势记录时阻止启动并保持原文件不变", async () => {
    mkdirSync(luckDir, { recursive: true });
    const path: string = join(luckDir, `${DAY}.json`);
  ...
  test("追加失败：缓冲保留、定时器重排、文件探测状态重置；故障排除后重试成功且不丢条目", async () => {
    // 把当天文件位置占成一个目录，让 BunFile.text 抛 EISDIR
    mkdirSync(join(luckDir, `${DAY}.json`), { recursive: true });
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "大吉", fortunePercent: 90.12 }));
  ...
  test("跨天刷盘失败时拒绝切日：宁可拒掉新日消息，也不拿旧日条目换 owner", async () => {
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "大吉", fortunePercent: 90.12, day: "2026-07-15" }));
    // 把旧日文件位置占成目录，让 BunFile.text 抛 EISDIR。
  ...
  test("flush 后 cleanupStaleLuckFiles 顺带删除非当日文件", async () => {
    ...
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "大吉", fortunePercent: 90.12, day: DAY }));
    await flushLuckAppends();
    expect(existsSync(join(luckDir, "2026-07-15.json"))).toBe(false);
    expect(existsSync(join(luckDir, `${DAY}.json`))).toBe(true);
  });

  test("每日维护先提交旧日缓冲，再切换 owner 并清理旧日文件", async () => {
    await handleLuckDrawMessage(luckMsg({
      key: "111",
      label: "大吉",
      fortunePercent: 90.12,
  ...
  test("每日维护刷盘失败时不切换 owner，也不提前清理旧文件", async () => {
    ...
    await expect(maintainLuckForDay(DAY)).rejects.toThrow(
  ...
  test("条数达到 FLUSH_MAX_ENTRIES 立即落盘，不等定时器", async () => {
    for (let i = 0; i < FLUSH_MAX_ENTRIES; i++) {
      await handleLuckDrawMessage(luckMsg({ key: `user${i}`, label: "小吉", fortunePercent: 60 }));
    }
...
describe("diskIO/luckFiles：追加持续失败的停摆诊断", () => {
  ...
  test("同一故障期内继续失败不重复告警（边沿触发，不刷爆 logs/）", async () => {
    ...
    for (let i = 0; i < LUCK_APPEND_STALL_ALERT_FAILURES * 3; i++) await flushLuckAppends();
  ...
  test("恢复后重新武装：下一次故障期会再告警一次", async () => {
    ...
    // 第二次故障期照常告警。
    await handleLuckDrawMessage(luckMsg({ key: "222", label: "小凶", fortunePercent: 39.99 }));
    repairDayFile();
    breakDayFile();
    for (let i = 0; i < LUCK_APPEND_STALL_ALERT_FAILURES; i++) await flushLuckAppends();
  ...
  test("诊断出口未装上时不算已告警：装上之后同一故障期仍会报出来", async () => {
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "大吉", fortunePercent: 90.12 }));
    breakDayFile();
  ...
  test("旧日刷不动时拒绝换 owner，但新一天的抽签滞留待补录而不是被丢掉", async () => {
    ...
    // 故障期内继续有人抽，同样滞留。
    await handleLuckDrawMessage(luckMsg({ key: "333", label: "吉", fortunePercent: 55, day: "2026-07-17" }));
  ...
  test("滞留区有上界：满了丢最旧的一条并记一行，绝不静默", async () => {
    await handleLuckDrawMessage(luckMsg({ key: "000", label: "大吉", fortunePercent: 90.12 }));
    breakDayFile();

    for (let i = 0; i < LUCK_DEFERRED_DRAW_MAX + 2; i++) {
      await handleLuckDrawMessage(luckMsg({ key: `k${i}`, label: "吉", fortunePercent: i, day: "2026-07-17" }));
    }
  ...
  test("跨日整体换 owner 时失败计数归零：旧 owner 的失败不算在新 owner 头上", async () => {
    await handleLuckDrawMessage(luckMsg({ key: "111", label: "大吉", fortunePercent: 90.12 }));
    breakDayFile();
    for (let i = 0; i < LUCK_APPEND_STALL_ALERT_FAILURES; i++) await flushLuckAppends();
