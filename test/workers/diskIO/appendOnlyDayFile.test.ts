...
import { chmodSync, mkdtempSync, rmSync, statSync, writeSync } from "node:fs";
...
async function readDay(day: string): Promise<unknown> {
  return JSON.parse(await Bun.file(join(dir, `${day}.json`)).text());
}

/** 仅验证明确选择截断修复的通用追加机制；领域状态恢复默认 fail closed。 */
async function openRepairableDay(day: string): Promise<DayFileState> {
  return { day, ...await openAppendOnlyFile(join(dir, `${day}.json`), undefined, true) };
}

describe("appendOnlyDayFile：按位置追加的字节层机制", () => {
  test("新建文件使用 0644，接管时保留部署方已有的 0600", async () => {
    const previousUmask: number = process.umask(0o077);
    try {
      const state: DayFileState = await openDayFile(dir, "2026-07-16", PERSISTED_FILE_MODE);
      await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("a", 1), mode: PERSISTED_FILE_MODE });
    } finally {
    ...
    await openDayFile(dir, "2026-07-16", PERSISTED_FILE_MODE);
    expect(statSync(path).mode & 0o777).toBe(0o600);
  });

  test("文件不存在 -> openDayFile 视为空文件", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    expect(state).toEqual({ day: "2026-07-16", size: 0, empty: true });
  });

  test("非对象顶层 JSON 会阻止接管并保持原始字节不变", async () => {
    const path: string = join(dir, "2026-07-16.json");
    for (const content of ["[]", "[{\"bad\":\"shape\"}]", "null", "\"text\"", "42"]) {
      await Bun.write(path, content);

      await expect(openDayFile(dir, "2026-07-16")).rejects.toThrow("must contain a top-level JSON object");
      expect(await Bun.file(path).text()).toBe(content);
    }
  });

  test("无法修复的语法损坏会阻止接管并保持原始字节不变", async () => {
    const path: string = join(dir, "2026-07-16.json");
  ...
  test("从空文件开始追加一条，结果是合法 JSON 且值正确", async () => {
    ...
    await appendToDayFile({ dir, state, chunk });

    expect(await readDay("2026-07-16")).toEqual({ "111": { label: "大吉", fortunePercent: 90.12 } });
    expect(state.empty).toBe(false);
  });

  test("单次 flush 内追加多条（逗号拼接的 chunk）：全部条目完整存在", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    const chunk: string = [
  ...
  test("多次独立 flush（跨多次 appendToDayFile 调用）累积追加，不覆盖已有内容", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("A", { label: "大吉", fortunePercent: 90.11 }) });
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("B", { label: "吉", fortunePercent: 75 }) });
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("C", { label: "尚可", fortunePercent: 50 }) });
  ...
  test("底层分段 short write 会循环到完整字节数组后才推进游标", async () => {
    ...
    expect(await readDay("2026-07-16")).toEqual({ A: { label: "大吉" }, B: { label: "小吉" } });
    expect(state.size).toBe(statSync(join(dir, "2026-07-16.json")).size);
  });

  test("已有文件的追加在成功返回前只执行一次 fsync，且顺序晚于完整 write", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    ...
    expect(await readDay("2026-07-16")).toEqual({ A: 1, B: 2 });
  });

  test("fsync 失败按追加失败上抛、重新探测游标并关闭 fd", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("A", 1) });
    let capturedFd: number | null = null;

    await expect(appendToDayFile({
      dir,
      state,
      chunk: serializeDayFileEntry("B", 2),
      sync: (fd) => {
  ...
  test("零字节写显式失败，不虚增 offset，并关闭文件描述符", async () => {
    ...
    })).rejects.toThrow("made no valid progress");

    expect(state.size).toBe(originalSize);
    expect(await readDay("2026-07-16")).toEqual({ A: 1 });
    expect(() => writeSync(capturedFd!, new TextEncoder().encode("x"), 0, 1, 0)).toThrow();
  });

  test("中途写异常会重新探测并修复文件，后续 append 不从虚假 offset 开始", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("A", { value: 1 }) });
    let call: number = 0;
    ...
    expect(await readDay("2026-07-16")).toEqual({ A: { value: 1 } });
    expect(state.size).toBe(statSync(join(dir, "2026-07-16.json")).size);
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("C", 3) });
    expect(await readDay("2026-07-16")).toEqual({ A: { value: 1 }, C: 3 });
  });

  test("重新 openDayFile（模拟进程重启）读到已有单条记录后，继续追加不会破坏旧内容", async () => {
    // 手工构造出与真实 memory/luck/2026-07-16.json 完全相同的字节内容
    const state1: DayFileState = await openDayFile(dir, "2026-07-16");
    await appendToDayFile({ dir, state: state1, chunk: serializeDayFileEntry("8791894415", { label: "小凶", fortunePercent: 39.99 }) });
    const raw: string = await Bun.file(join(dir, "2026-07-16.json")).text();
    expect(raw).toBe('{\n  "8791894415": {\n    "label": "小凶",\n    "fortunePercent": 39.99\n  }\n}');

    // 模拟重启：新的 DayFileState 通过重新探测磁盘现有文件得到
    const state2: DayFileState = await openDayFile(dir, "2026-07-16");
    expect(state2.empty).toBe(false);
    await appendToDayFile({ dir, state: state2, chunk: serializeDayFileEntry("222", { label: "大凶", fortunePercent: 5 }) });
  ...
  test("大批量条目（超过 FLUSH_MAX_ENTRIES 量级）连续追加，全部完整且顺序无关", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    const total = 500;
    for (let i = 0; i < total; i++) {
      await appendToDayFile({ dir, state, chunk: serializeDayFileEntry(`user${i}`, { label: "小吉", fortunePercent: 60 + (i % 8) }) });
    }
    const result: Record<string, unknown> = await readDay("2026-07-16") as Record<string, unknown>;
  ...
  test("截断修复：断电截断在最后一条记录中间时，能裁掉残片并保留此前的完整记录", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("A", { label: "大吉", fortunePercent: 90.11 }) });
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("B", { label: "小凶", fortunePercent: 39.99 }) });
    const full: string = await Bun.file(join(dir, "2026-07-16.json")).text();
    ...
    const recovered: DayFileState = await openRepairableDay("2026-07-16");
    expect(recovered.empty).toBe(false);
    const parsedAfterRepair: unknown = await readDay("2026-07-16");
    // 截断恢复必须保留残片之前的完整记录 A。
    expect((parsedAfterRepair as any).A).toEqual({ label: "大吉", fortunePercent: 90.11 });

    // 修复后继续追加应仍能产出合法 JSON。
    await appendToDayFile({ dir, state: recovered, chunk: serializeDayFileEntry("C", { label: "尚可", fortunePercent: 50 }) });
    expect((await readDay("2026-07-16") as any).C).toEqual({ label: "尚可", fortunePercent: 50 });
  });

  test("截断修复：重复 key 的 null tombstone 后发生撕裂时，保留 tombstone 作为最后值", async () => {
    const state: DayFileState = await openDayFile(dir, "2026-07-16");
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("K", { revision: 1 }) });
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("R", { revision: 1 }) });
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("K", null) });
    await appendToDayFile({ dir, state, chunk: serializeDayFileEntry("N", { revision: 1, payload: "会被截断" }) });

    const path: string = join(dir, "2026-07-16.json");
    const full: string = await Bun.file(path).text();
    const tornEntryStart: number = full.lastIndexOf('"N"');
    await Bun.write(path, full.slice(0, tornEntryStart + 20));

    const recovered: DayFileState = await openRepairableDay("2026-07-16");
    expect(await readDay("2026-07-16")).toEqual({ K: null, R: { revision: 1 } });

    await appendToDayFile({ dir, state: recovered, chunk: serializeDayFileEntry("C", true) });
    expect(await readDay("2026-07-16")).toEqual({ K: null, R: { revision: 1 }, C: true });
  });

  test("截断修复：早期成员损坏后仍只保留损坏前的最后有效前缀", async () => {
    const path: string = join(dir, "2026-07-16.json");
    const later: string[] = [];
    ...
    expect(await readDay("2026-07-16")).toEqual({ valid: { value: 1 } });
  });

  test("截断修复：断电截断发生在第一条记录写入之前（文件只剩一个 \"{\"），修复出的空对象要被正确判成 empty，" +
    "否则下一次追加会误判成「非空、按位置追加」写出非法 JSON（回归：曾导致这条记录永久丢失的级联损坏）", async () => {
    const path = join(dir, "2026-07-16.json");
    ...
    const state3: DayFileState = await openDayFile(dir, "2026-07-16");
    expect(state3.empty).toBe(false);
    await appendToDayFile({ dir, state: state3, chunk: serializeDayFileEntry("222", { label: "小吉", fortunePercent: 60 }) });
    expect(await readDay("2026-07-16")).toEqual({
