...
const { describeGeminiVision, generateGeminiText } = await import("../../../packages/aiChat/gemini/text");
const {
...
describe("纯文本生成", () => {
  test("系统提示词进 systemInstruction，待处理内容进单段 user text", async () => {
    ...
    const body = (requestGeminiTextResult.mock.calls[0]![0] as () => GenerateContentParameters)();
    expect(body.model).toBe(getGeminiDeploymentConfig().models.summary);
    ...
    expect(requestGeminiTextResult.mock.calls[0]![1]).toBe("AI summarize API");
  });
  ...
  test("清洗函数原样透传给底层，由领域侧决定截断口径", async () => {
    ...
    expect(requestGeminiTextResult.mock.calls[0]![2]).toBe(normalize);
  });
});
...
describe("视觉描述", () => {
  test("图片以 inlineData 内联，描述指令跟在图片之后", async () => {
    ...
    const body = (requestGeminiTextResult.mock.calls[0]![0] as () => GenerateContentParameters)();
    expect(body.model).toBe(getGeminiDeploymentConfig().models.media);
