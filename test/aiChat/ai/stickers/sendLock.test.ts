...
import { createStickerSendLock } from "../../../../packages/aiChat/ai/stickers/sendLock";

/**
 * aiChat/ai/stickers/sendLock.ts 的纯逻辑单测：锁的抢占/重入/互斥/释放语义。与
 * send_sticker 执行路径的整合行为（拒绝发送、收回挡位）在
 * test/aiChat/ai/stickers.test.ts 里覆盖。所有用例注入独立 Map，不碰 Worker 全局
 * 的 stickerSendLocks。
 */

describe("aiChat/ai/stickers/sendLock createStickerSendLock", () => {
