 ...
 * 标签的；同时覆盖单声道/立体声的 Xing 偏移差异，以及 ID3v2.3/2.4 尾部标志位
 * 的区别。认不出帧头或声明长度越界时返回 null。
 */
...
describe("MP3 元信息解析", () => {
  ...
  test("ID3v2.4 的可选尾部也要跳过：它与标签头等长且不计入头里那个长度", () => {
    ...
    bytes.set(prefix, 0);
    // 尾部这 10 字节里放一个可被识别为 128kbps/44.1kHz 帧头的字节序列，
    // 用来检验跳过尾部长度后扫描不会误停在这里。
