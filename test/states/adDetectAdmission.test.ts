...
describe("投递闸 admitAdCandidate", () => {
  ...
  test("缓存明确认得的管理员挡在额度之外；频道马甲不走这道闸", () => {
    ...
    expect(admitAdCandidate({ ...CANDIDATE, knownAdmin: true })).toBe("ignore");
    // 频道马甲没有「群成员」身份，管理员表里不会有它。
    expect(admitAdCandidate({ ...CANDIDATE, knownAdmin: true, isChannel: true })).toBe("accept");
  });
...
describe("在途闸 admitAdDispatch", () => {
  test("按全局在途数判定，不按群分配", () => {
    expect(admitAdDispatch({ inFlight: 0 })).toBe("dispatch");
    expect(admitAdDispatch({ inFlight: AD_DETECT_MAX_IN_FLIGHT - 1 })).toBe("dispatch");
    expect(admitAdDispatch({ inFlight: AD_DETECT_MAX_IN_FLIGHT })).toBe("saturated");
  });
});
