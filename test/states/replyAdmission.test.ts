...
describe("admitTrigger：并发未满且队列已空", () => {
  for (const kind of ALL_KINDS) {
    test(`kind=${kind} 且 activeRounds 低于上限 → startRound`, () => {
      ...
      expect(decision).toBe("startRound");
    });
  }

  test("activeRounds 为 0 → startRound", () => {
    expect(admitTrigger({ activeRounds: 0, queueSize: 0, kind: "direct" })).toBe("startRound");
  });
});
...
describe("admitTrigger：并发未满但队列非空", () => {
  ...
  test("kind=direct → enqueue，不插队", () => {
    expect(admitTrigger({ activeRounds: 0, queueSize: 1, kind: "direct" })).toBe("enqueue");
  });

  test("kind=random → dropSilently：随机触发本就不排队", () => {
    expect(admitTrigger({ activeRounds: 0, queueSize: 1, kind: "random" })).toBe("dropSilently");
  });

  test("队列已满 → enqueueOverflow", () => {
    expect(admitTrigger({ activeRounds: 0, queueSize: REPLY_TRIGGER_QUEUE_MAX, kind: "direct" }))
      .toBe("enqueueOverflow");
  });
});

describe("admitTrigger：并发已满（activeRounds === 上限，边界值）", () => {
  test("kind=random → dropSilently，即使队列全空", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: 0, kind: "random" })).toBe("dropSilently");
  });

  test("kind=mediaRandom → dropSilently，即使队列全空", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: 0, kind: "mediaRandom" })).toBe("dropSilently");
  });

  test("kind=direct 且队列未满 → enqueue", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: REPLY_TRIGGER_QUEUE_MAX - 1, kind: "direct" })).toBe("enqueue");
  });

  test("kind=mediaDirect 且队列未满 → enqueue", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: 0, kind: "mediaDirect" })).toBe("enqueue");
  });

  test("kind=direct 且队列已达上限（边界值）→ enqueueOverflow", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: REPLY_TRIGGER_QUEUE_MAX, kind: "direct" })).toBe("enqueueOverflow");
  });

  test("kind=mediaDirect 且队列已达上限 → enqueueOverflow", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: REPLY_TRIGGER_QUEUE_MAX, kind: "mediaDirect" })).toBe("enqueueOverflow");
  });

  test("kind=direct 且队列超过上限（异常输入防御）→ enqueueOverflow", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT, queueSize: REPLY_TRIGGER_QUEUE_MAX + 5, kind: "direct" })).toBe("enqueueOverflow");
  });
});

describe("admitTrigger：并发超过上限（异常输入防御，与 === 上限行为一致）", () => {
  test("kind=random → dropSilently", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT + 3, queueSize: 0, kind: "random" })).toBe("dropSilently");
  });

  test("kind=direct 且队列未满 → enqueue", () => {
    expect(admitTrigger({ activeRounds: REPLY_ROUND_MAX_CONCURRENT + 3, queueSize: 0, kind: "direct" })).toBe("enqueue");
  });
});
