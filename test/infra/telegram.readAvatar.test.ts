...
const ABSENT: CurrentAvatarResult = { status: "permanent-failure" };
const FAILED: CurrentAvatarResult = { status: "transient-failure" };

/** 只关心发送源的断言复用这一层拆包，不重复展开三态结果。 */
async function readPhoto(target: User | number, signal: AbortSignal): Promise<string | Uint8Array | undefined> {
  const result: CurrentAvatarResult = await readCurrentAvatar(target, signal);
  return result.status === "ok" ? result.photo : undefined;
}
...
test("历史列表匹配当前头像，直接返回可复用 file_id，不下载或猜测首张历史", async (): Promise<void> => {
  ...
  expect(await readCurrentAvatar(user, signal)).toEqual({ status: "ok", identity: user, photo: "reusable-current" });
...
test("无可复用头像时下载当前 ChatPhoto，不把只供下载的 file_id 当成可发送图片", async (): Promise<void> => {
  const signal: AbortSignal = new AbortController().signal;
  expect(await readCurrentAvatar(user, signal)).toEqual({ status: "ok", identity: user, photo: new Uint8Array([1, 2, 3]) });
...
test("下载失败时可以继续读取已核实用户的公开头像", async (): Promise<void> => {
  ...
  expect(await readCurrentAvatar(user, new AbortController().signal)).toEqual({ status: "ok", identity: user, photo: new Uint8Array([4, 5]) });
});
...
test("频道使用 getChat 的当前身份与头像；下载失败仅访问当前公开用户名", async (): Promise<void> => {
  ...
  const signal: AbortSignal = new AbortController().signal;
  expect(await readCurrentAvatar(channel.id, signal)).toEqual({ status: "ok", identity: channel, photo: new Uint8Array([1, 2, 3]) });
...
test("频道身份查询失败或返回不符时不猜测用户名，也不下载其它身份的图片", async (): Promise<void> => {
  const signal: AbortSignal = new AbortController().signal;
  getChat.mockImplementationOnce(async (): Promise<any> => { throw new Error("chat inaccessible"); });
  expect(await readCurrentAvatar(-10042, signal)).toEqual(FAILED);
  getChat.mockImplementationOnce(async (): Promise<any> => ({ id: -10042, type: "supergroup", username: "old_name" }));
  expect(await readCurrentAvatar(-10042, signal)).toEqual(ABSENT);
...
test("历史头像未匹配当前身份时只下载当前头像", async (): Promise<void> => {
  ...
  expect(await readPhoto(user, new AbortController().signal)).toEqual(new Uint8Array([1, 2, 3]));
...
for (const stage of ["chat", "photos"] as const) {
  test(`${stage} 查询期间取消，不消费迟到头像或进入下载兜底`, async (): Promise<void> => {
    ...
    const task: Promise<CurrentAvatarResult> = readCurrentAvatar(user, controller.signal);
    await started.promise;
    controller.abort();
    pending.resolve(stage === "chat"
      ? { photo: { big_file_id: "active-photo", big_file_unique_id: "active-unique" } }
      : currentPhotos());
    expect(await task).toEqual(FAILED);
    expect(download).not.toHaveBeenCalled();
    expect(web).not.toHaveBeenCalled();
...
test("update 在头像查询期间取消时继续传播取消，不降级为下载", async (): Promise<void> => {
  ...
  const task: Promise<CurrentAvatarResult> = runWithUpdateAbortSignal(controller.signal,
    (): Promise<CurrentAvatarResult> => readCurrentAvatar(user, new AbortController().signal));
