...
import { prepareHImageAddPermissionMigration, readMigrationFileRecord } from "../../scripts/migrateHImageAddPermission";
import type { HImageAddPermissionMigrationResult } from "../../scripts/migrateHImageAddPermission";
...
beforeEach(async () => {
  root = await mkdtemp(join(TEST_DATA_ROOT, "h-image-add-permission-migration-"));
  ...
  try {
    ...
    for (const [index, permissions] of [SUPER_ADMIN_WHITELIST_PERMISSIONS, DEFAULT_WHITELIST_PERMISSIONS].entries()) {
      const { isCanAddHImage: _add, ...previous } = permissions;
...
test("直接迁移仅增加收图权限，原有全部权限开启的成员获得新权限", async () => {
  const before: string = new Bun.CryptoHasher("sha256").update(await Bun.file(path).bytes()).digest("hex");
  const result: HImageAddPermissionMigrationResult = await prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "output") });
  expect(result).toMatchObject({ sourceSchema: 10, targetSchema: 11, enabledPermissions: 1, disabledPermissions: 1 });
  ...
  try {
    ...
    expect(client.query("SELECT id, json_extract(policy, '$.permissions.isCanAddHImage') AS enabled FROM permission_list ORDER BY id").all()).toEqual([{ id: 1, enabled: 1 }, { id: 2, enabled: 0 }]);
...
test.each(["version", "lineage", "permission", "context"])("非法 %s 不产生 ready，源副本保持不变", async (kind) => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "output") })).rejects.toThrow();
...
test("不覆盖既有输出，中断后在新输出目录重跑", async () => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "interrupted") })).rejects.toThrow();
  expect(await Bun.file(join(root, "interrupted/incomplete.json")).text()).toBe("preserve");
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "retry") })).resolves.toMatchObject({ enabledPermissions: 1 });
});
...
test.each([0, 1])("outbox 容量与生产启动门禁一致：上限加 %s", async (extra: number): Promise<void> => {
  ...
  const migration = prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: output });
  if (extra === 0) await expect(migration).resolves.toMatchObject({ targetSchema: 11 });
...
test("任意一项原有权限为 false 时不授予新权限，保留原权限和元数据", async () => {
  ...
  try {
    ...
    for (const key of WHITELIST_PERMISSION_KEYS) {
      if (key === "isCanAddHImage") continue;
      const { isCanAddHImage: _add, ...previous } = SUPER_ADMIN_WHITELIST_PERMISSIONS;
      previous[key] = false;
  ...
  const result = await prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "output") });
  ...
  try {
    for (const [id, value] of expected) {
      const row = output.query<{ original: string; enabled: string }, [number]>("SELECT json(jsonb_remove(policy, '$.permissions.isCanAddHImage')) AS original, json_type(policy, '$.permissions.isCanAddHImage') AS enabled FROM permission_list WHERE id = ?").get(id)!;
...
test("接受前次迁移产出的历史 JSONB 基础谱系", async () => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "output") }))
    .resolves.toMatchObject({ sourceSchema: 10, targetSchema: 11, enabledPermissions: 1 });
});
...
test.each(["string", "number", "unknown", "already-present", "old-version", "extra-lineage"])("拒绝来源异常 %s，不能通过迁移补齐或覆盖", async (kind) => {
  ...
  try {
    ...
    if (kind === "already-present") client.run("UPDATE permission_list SET policy = jsonb_set(policy, '$.permissions.isCanAddHImage', jsonb('false'))");
    if (kind === "old-version") client.run("UPDATE storage_metadata SET data = jsonb('{\"version\":9}')");
...
test("完成后的 v11 不作为 v10 来源再次授权", async () => {
  const result = await prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "output") });
  const outputPath: string = join(result.outputRoot, "database/storage.sqlite");
  const client: Database = new Database(outputPath);
  try { client.run("UPDATE permission_list SET policy = jsonb_set(policy, '$.permissions.isCanAddHImage', jsonb('false'))"); } finally { client.close(true); }
  const before = await Bun.file(outputPath).bytes();
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: result.outputRoot, outputRoot: join(root, "repeat") })).rejects.toThrow("schema version 10");
...
test("迁移事务失败时回滚权限位与谱系，保留不完整产物", async () => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: join(root, "output") })).rejects.toMatchObject({ cause: expect.objectContaining({ message: "injected migration failure" }) });
  const output: Database = new Database(join(root, "output/database/storage.sqlite"), { readonly: true });
  try {
    expect(output.query("SELECT json_extract(data, '$.version') AS version FROM storage_metadata").get()).toEqual({ version: 10 });
    expect(output.query("SELECT count(*) AS count FROM permission_list WHERE json_type(policy, '$.permissions.isCanAddHImage') IS NOT NULL").get()).toEqual({ count: 0 });
    expect(output.query("SELECT max(created_at) AS latest FROM __drizzle_migrations").get()).toEqual({ latest: 20_260_915_010_000 });
  } finally { output.close(true); }
  expect(await Bun.file(join(root, "output/ready.json")).exists()).toBeFalse();
...
test("副本哈希读取中失败时拒绝完成并保留 incomplete 清单", async (): Promise<void> => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: output })).rejects.toThrow(injected);
...
test("最终 ready 写入失败时保留 incomplete 清单与完整源备份", async () => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: output })).rejects.toThrow("injected ready write failure");
  expect(await Bun.file(join(output, "ready.json")).exists()).toBeFalse();
  expect(await Bun.file(join(output, "incomplete.json")).exists()).toBeTrue();
  expect(await Bun.file(path).bytes()).toEqual(before);
});
...
test("源文件在复制期间变化时，最终哈希复核拒绝发布 ready", async (): Promise<void> => {
  ...
  await expect(prepareHImageAddPermissionMigration({ sourceRoot: source, outputRoot: output })).rejects.toThrow("$snapshot");
  expect(await Bun.file(join(output, "ready.json")).exists()).toBeFalse();
  expect(await Bun.file(join(output, "incomplete.json")).exists()).toBeTrue();
});
