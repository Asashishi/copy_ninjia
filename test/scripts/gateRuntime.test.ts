...
async function fixture(packageManager: unknown = "bun@1.4.1"): Promise<string> {
  ...
  record.hotPathProfileGate.calibration.runtime.bunVersion = "1.4.1";
...
test("manifest、Bun 版本和构建一致时通过，不要求历史 fullSuite 更新", async (): Promise<void> => {
  expect(await collectRuntimeCalibrationProblems({
    projectRoot: await fixture(), runtime: { bunVersion: "1.4.1", bunRevision: "expected" },
  })).toEqual([]);
});
test("manifest 漂移或缺少 packageManager 时在昂贵门禁前失败", async (): Promise<void> => {
  for (const manager of ["bun@1.4.0", null]) {
    expect(await collectRuntimeCalibrationProblems({
      projectRoot: await fixture(manager), runtime: { bunVersion: "1.4.1", bunRevision: "expected" },
    })).toEqual([expect.stringContaining("$.packageManager must equal bun@1.4.1")]);
  }
});
test("版本或同版本不同构建必须重新校准", async (): Promise<void> => {
  for (const runtime of [
    { bunVersion: "1.4.0", bunRevision: "expected" },
    { bunVersion: "1.4.1", bunRevision: "different" },
  ]) {
