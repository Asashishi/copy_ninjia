...
import type { CronAction, CronSendNeeds, CronTask } from "../../packages/types/cron";
...
function task(actions: readonly CronAction[]): CronTask {
  return {
    name: "all-groups",
    chatId: "all",
...
describe("chat_id: \"all\" 的发送权限", () => {
  test("动作类型决定需要哪几项发送权限", () => {
    expect(sendNeedsOf([{ type: "send_message", content: "hi" }])).toEqual(TEXT_ONLY);
    expect(sendNeedsOf([
      { type: "send_image", content: undefined, source: { kind: "random", directory: null } },
  ...
  test("只看已启用的群，按 chat id 升序；普通成员读群默认权限，缺权限与查询失败都计入跳过", async () => {
    ...
    const targets = await resolveCronGroupTargets(task([
      { type: "send_message", content: "hi" },
      { type: "send_image", content: undefined, source: { kind: "url", url: "https://e.com/a.png" } },
  ...
  test("取消后不再查询后面的群", async () => {
    ...
    controller.abort();
    expect(await resolveCronGroupTargets(task([{ type: "send_message", content: "hi" }]), controller.signal))
