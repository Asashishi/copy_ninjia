...
describe("cron 调度", () => {
  test("非法停机预算在关闭接纳之前拒绝", async () => {
    ...
    for (const budget of [-1, Number.NaN, Number.POSITIVE_INFINITY]) {
      await expect(drainCronScheduler(budget)).rejects.toThrow(
        new RangeError("Cron drain timeout must be a non-negative finite number.")
      );
    }
    expect(runtime().accepting).toBeTrue();
  });
  ...
  test("原样对账保留句柄；变更或删除的任务撤销，在途一轮据此停下", async () => {
    ...
    cronConfigCache.current = [task(), task({ name: "other", chatTargets: { kind: "list", chatIds: [-2002] } })];
    reconcileCronSchedule();
    ...
    cronConfigCache.current = [task({ name: "other", chatTargets: { kind: "list", chatIds: [-2002] } })];
    reconcileCronSchedule();
