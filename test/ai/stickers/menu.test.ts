...
/**
 * 贴纸包菜单的记忆化（packages/ai/tools/stickers.ts 的 buildStickerPackMenu）。
 *
...
// ai/tools/stickers.ts 从领域入口 `../stickers` 取 getStickerSet，而那个入口是
// `export *`：只替换 sets.ts 会让入口少掉其余导出，因此连同入口一起透传。
const realSets = { ...(await import("../../../packages/ai/stickers/sets")) };
const realStickers = { ...(await import("../../../packages/ai/stickers")) };
mock.module("../../../packages/ai/stickers/sets", () => ({
  ...realSets,
  getStickerSet: getStickerSetMock,
}));
mock.module("../../../packages/ai/stickers", () => ({
  ...realStickers,
  getStickerSet: getStickerSetMock,
}));
