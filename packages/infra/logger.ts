...
function currentSecrets(): string[] {
  ...
  const telegramToken: string | undefined = telegramConfigCache.current?.botToken;
  if (telegramToken !== undefined) secrets.push(telegramToken);
  const adDetectApiKey: string | undefined = adDetectAgentConfigCache.current?.apiKey;
...
function emit(level: LogLevel, args: unknown[]): void {
  ...
  const serializedArgs: unknown[] = args.map(
