 ...
 * 最终失败记一行英文错误日志并中止这一落点剩余的动作。任务被热重载撤销或停机时，当前
...
import type { CronAction, CronAllChats, CronDeliveryOutcome, CronDestination, CronGroupTargets, CronTaskSchedule } from "../types/cron";

/** 一个落点上的一次投递所需的上下文；每个落点建一次，动作与重试共用。 */
interface CronDeliveryContext {
  readonly schedule: CronTaskSchedule;
  readonly destination: Readonly<CronDestination>;
...
async function deliverWithRetries(
  { schedule, destination, signal }: CronDeliveryContext,
  ...
  for (let attempt: number = 0; ; attempt++) {
    ...
    const outcome: CronDeliveryOutcome = await deliverCronAction(destination, action, signal);
...
/**
 * 向一个落点顺序投递全部动作。失败记一行日志并中止该落点剩余的动作；返回 false 表示
 * 本轮已被撤销或取消，调用方应整轮收场。
 */
async function runActions(context: CronDeliveryContext): Promise<boolean> {
  const { schedule, destination, signal, perChat }: CronDeliveryContext = context;
  ...
  for (let index: number = 0; index < actions.length; index++) {
    ...
    if (result.kind === "failed") {
      logger.error(perChat
        ? `Cron task "${schedule.task.name}" action #${index + 1} (${action.type}) failed in chat ${destination.chatId} ` +
...
export async function runCronRound(schedule: CronTaskSchedule, signal: AbortSignal): Promise<void> {
  ...
  if (chatId !== CRON_ALL_CHATS) {
    await runActions({ schedule, destination: { chatId, messageThreadId: schedule.task.messageThreadId }, signal, perChat: false });
    return;
  }
  ...
  for (let index: number = 0; index < targets.chatIds.length; index++) {
    ...
    const destination: Readonly<CronDestination> = { chatId: targets.chatIds[index]!, messageThreadId: undefined };
    if (!await runActions({ schedule, destination, signal, perChat: true })) return;
  }
