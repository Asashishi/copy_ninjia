 ...
 * 公告与欢迎语、验证提醒等真正的聊天消息共用 grammY 的同群发送桶
 * （20 msg/min、maxConcurrent 1）。协同突袭里几十个号各自越过阈值，会让真正
 * 有交互价值的提醒排在过时公告后面，群里还会迟到地冒出几十条点名成员的消息。
 * kick/restrict 已有独立 429 域，不受这个消息桶影响；这里丢的是过时提示，不是
 * 安全动作。
 *
 * 超时的公告被丢掉时也就给验证提醒等功能性消息腾出了发送位。取 30 秒：正常群里队列很短，30 秒
 * 绰绰有余；真排到 30 秒之后，
