 ...
 * 35 × `DEEPSEEK_REQUEST_TIMEOUT_MS`(20 秒) ≈ 700 个，算上重试还要翻几倍，
...
/**
 * 命中样本里随每条消息一起记录的「被引用段」与「被回复原文」的最大字符数。
 *
 * 这两样**只进样本文件、绝不进送检文本**（那是别人的内容，并进去等于让引用
 * 广告吐槽的群友替广告主背锅，见 AD_DETECT_SYSTEM_PROMPT 上方的约束与
 * docs/04-invariants.md）。留在样本里是另一回事：人回头看「这条为什么被判成
 * 广告」时，往往正需要知道它当时在回谁、引了什么。
...
export const AD_DETECT_SYSTEM_PROMPT: string =
  ...
  "A. 三件套：收益承诺 + 低门槛 + 联系方式（「日入过千」+「无需经验、不用押金」+「加V xxx」）。" +
  "三样凑齐几乎就是广告的标准骨架；只占一样通常不是。\n" +
  ...
  "加群。广告最终一定要把人引走；没有任何落点的内容通常只是闲聊。\n" +
 ...
 * 入群验证镜像给出（见 antiRaid/adDetect.ts），模型无从自行判断——群聊转录里
