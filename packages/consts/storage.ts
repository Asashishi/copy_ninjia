 ...
 *
 * 这道闸只拦**写**：group 或 other 拿到 w 位一律拒绝启动，因为那意味着别的账号
 * 能改运行状态；读侧放开到 0755，按单租户部署基线处理。
 *
 * `memory/` 下的文件本身是 0644（见 docs/cn/07-operations.md），所以群聊逐字记录
 * 的读取边界由数据根目录权限提供；0755 会允许同机本地账号读取。多租户或有非
 * 特权登录用户的机器上，部署方必须自己把数据根收回 0750——预检只保证不比这更
 * 宽，不替谁做决定。`database/` 不受影响，仍走 IDENTITY_DATABASE_DIRECTORY_MODE
 * 的 0770。
 */
