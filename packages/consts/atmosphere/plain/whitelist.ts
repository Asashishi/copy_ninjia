...
export const WHITELIST_PERMISSION_HELP: Readonly<Record<WhitelistPermissionKey, string>> = {
  ...
  isCanBlock: "允许使用 /block enable 将用户加入永久黑名单并在受管群封禁。",
  isCanUnBlock: "允许使用 /block disable 移除永久黑名单记录并解除受管群封禁。",
...
export const WHITE_COMMAND_TEXTS: Readonly<WhiteCommandTexts> = {
  ...
  blocked: (targetLabel: string): string => `${targetLabel} 仍在黑名单中，请先使用 /block disable 解除。`,
