 ...
 * reloadAgentDeploymentConfig），lane 数因此仍不超过五条。Worker 崩溃时 isolate
...
/**
 * 丢弃全部能力门面，下一次取用按当前配置快照重建；在途请求继续持有旧门面直至
 * 结算。agent.json 热重载与下方全量清理共用。
 */
export function resetAiProviderFacades(): void {
