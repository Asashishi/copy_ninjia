 ...
 * telegramConfigCache 属 packages/config/telegram.ts；adDetectAgentConfigCache 与
...
/**
 * config/telegram.json 的线程内只读快照；模块初始化时读盘填充，进程或 Worker
 * 重建后重读。不热重载，因此容量恒为一个对象且无需淘汰。
 */
