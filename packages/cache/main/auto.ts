 ...
 * 在冷却到期后回收；达到硬顶时新增前先补扫一次，仍满则 fail closed 放弃本次随机
 * 回复，不淘汰仍生效的冷却。
 */
 ...
 * validFrom/validUntil 记录最近清扫确认的满表有效区间；区间内无需重复遍历。
 * 满表条目换代、清扫后不足硬顶或整表清空时失效；两个数值固定占用，不跨线程重放。
 */
export const userReplyTriggerSweepState: {
  timer: ReturnType<typeof setTimeout> | null;
  validFrom: number;
  validUntil: number;
} = { timer: null, validFrom: 0, validUntil: Number.NEGATIVE_INFINITY };
