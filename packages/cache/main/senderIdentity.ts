...
/** owner: main。发送者身份缓存（packages/users/senderIdentity.ts）的内存状态；进程重启后从空表
 * 开始，由后续发送者消息重新填充。 */
 ...
 * 清理：满载时 users/senderIdentity.ts 的 deleteAlias 按插入序淘汰最旧 alias，
 * 发送者改名/去名/换绑时同步撤销旧 alias；没有整表清空路径，进程重启归零。 */
