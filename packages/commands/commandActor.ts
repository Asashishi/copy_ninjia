...
import { channelIdentity, userIdentity } from "../users/visibleSender";

/**
 * 解析命令对外可见的发起身份。sender_chat（频道马甲/频道帖）优先于 from，
 * 否则频道白名单会被 Telegram 附带的匿名服务用户误判；普通用户则回退到 from。
 *
 * 判定口径必须与 users/visibleSender.ts 的 visibleSenderChat 逐字一致（两种身份的
 * 形状本身也取自同一模块的 channelIdentity / userIdentity）：`/qa set`
