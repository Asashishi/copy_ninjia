 ...
 * 注解给出朗读风格（基础风格之后拼上本句语气）；`response_format` 只声明要音频，
 * 服务端对非流式请求默认回 `audio/wav`（24 kHz 单声道 16 bit PCM）；
 * `generation_config` 指定音色与采样温度。
 * 音色取自 config/agent.json 的 `agent.tts.voice`，经 `speech_config[].voice` 原样
 * 传入；风格与温度取自 consts/aiChat/gemini.ts。
 *
...
/** `interactions.create` 交回的结果里本次真正要读的字段，一次收窄成「音频在不在」。 */
 ...
 * 构造携带台词与朗读风格的文本块。
 * @param text 要念出来的台词。
 * @param tone 本句说话语气；给出时拼在基础风格 GEMINI_SPEECH_STYLE 之后。
 */
function speechTextContent(text: string, tone: string | undefined): Interactions.TextContent {
  const style: string = tone === undefined ? GEMINI_SPEECH_STYLE : GEMINI_SPEECH_STYLE + tone;
...
export async function synthesizeGeminiSpeech({ text, tone, signal }: AiSpeechRequest): Promise<SynthesizedSpeech | null> {
  ...
  try {
    ...
    // 模型名与音色在 try 内部求值，部署配置读取失败同样归一成一次普通失败。
    ...
    interaction = await raceAbortOrThrow(
      client.interactions.create(
        {
          model: tts.model,
          input: [{ type: "user_input", content: [speechTextContent(text, tone)] }],
