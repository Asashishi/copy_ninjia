 ...
 * 从音频字节里读出时长与码率。
 *
 * Lyria 的响应只带 `sample_rate` 和 `channels`，没有时长；sendAudio 的
 * `duration` 参数即取自本模块的解析结果。大小由 Telegram 按上传字节自算，
 * 不在本模块处理范围内。
 *
 * 只解析 MP3 容器；认不出的容器一律返回 null，调用方按「这两项未知」降级，
 * 不填一个猜测出来的值。
 *
 ...
 *
 * 两处必须显式处理，否则扫描会从**标签内部**起步——标签里的用户文本和封面图
 * 容易凑出偶然的 11 位同步字，一旦同时蒙对版本/层/码率/采样率四项，就会算出
 * 一个看着正常却完全错误的时长：
 *
 * 1. **2.4 的可选尾部**（标志位 0x10）与标签头等长，且不计入头里那个长度，
 *    漏加就把这 10 字节当成音频。该位仅 2.4 定义，更早主版本此位保留为 0，
 *    因此还要看主版本再决定认不认。
 * 2. **声明长度覆盖到文件末尾**（畸形或截断）时没有可解析的音频，把起点收到
 *    文件末尾让扫描空手而归，调用方按「这两项未知」降级。
 */
