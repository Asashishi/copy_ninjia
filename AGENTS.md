...
- 状态机拥有的按钮消息（入群验证按钮、`/qa set` 表单）不挂固定延迟删除，只由按钮、状态机或 teardown 路径删除；新增此类消息必须在 `bun run check:conventions` 里登记对应豁免，并把发送收在唯一一个边界函数里。
- 仅 `/permission help`、`/permission query` 的权限看板、`/qa query` 的问答看板、问答直答的答案、成功的中文动作命令结果（如 `/咬`、`/揪住`）、`/h_image` 的结果图片，和 `cron.json` 定时任务发出的消息允许长期保留。
