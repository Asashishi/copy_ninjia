...
- `scripts/` 中的冷迁移脚本按**迁移**计数，与 Release 数量无关：每条边只覆盖「上一次迁移产出的格式 → 本次迁移产出的格式」的直接迁移及该迁移自身的中断续跑。同一份持久化数据被多次迁移时只保留最近那一次的边，旧边连同它的测试一并删除；不得累积更早格式或跨多次迁移的兼容链。落后于上一次迁移的部署必须先分阶段升级到那一次迁移的产出格式，未知谱系一律拒绝。
- 配置和状态全部就位后才能启动服务。
...
- 字符串字段的首尾空白按规范化去掉（trim），不视为配置错误；去掉后为空或形态不符仍按非法处理。
...
- 状态机拥有的消息（入群验证按钮、`/qa set` 表单、`/wed` 结果图片及其按钮、gag 公告与发言入口提示）不挂固定延迟删除，只由按钮、状态机或 teardown 路径删除；新增此类消息必须在 `bun run check:conventions` 里登记对应豁免，并把发送收在唯一一个边界函数里。
- 仅以下内容允许长期保留：`/permission help`、`/permission query` 的权限看板，`/qa query` 的问答看板，问答直答的答案，成功的中文动作命令结果（如 `/咬`、`/揪住`），`/h_image` 的结果图片（固定以 Telegram 剧透遮罩发送），`cron.json` 定时任务发出的消息（`send_image` 按 `is_blurred` 决定是否遮罩），以及会话性输出（复读、翻译、AI 回复、洗澡回复）。
- 由用户命令或交互触发的群内消息落在触发消息所在的论坛话题（`message_thread_id`），长期保留的消息、状态机消息与 30 秒自删的命令提示都适用：主线程命令提示由统一发送边界按当前 update 的触发话题补齐，脱离 update 作用域的发送必须显式带上触发话题。bot 主动发出的提示（刷屏禁言公告、广告警告与封禁播报、入群验证提醒、私密模式公告）与 `cron.json` 定时任务不带话题（开了话题的群里落在 General）。
...
- 断言涉及部署配置、示例配置（`config_example/`）或代码常量的取值时，必须从对应的配置解析结果、holder 或常量模块读取，不得在测试里写死具体值；只有测试自己构造的夹具和 mock 可以使用字面量。

### 注释、日志与文档

- 注释和 JSDoc 使用中文，只描述现行代码的职责、调用方式、输入输出、状态变化、执行顺序、边界和不变量。
- 注释和文档只写当前实现如何工作、如何配置、如何操作和如何验证；不得记录选择原因、备选方案、废弃实现、修复前行为、历史性能对比或提交过程。
- 每次修改代码，必须在同一改动中检查并同步更新所有受影响的注释和 JSDoc，包括改动位置、调用方、类型定义、常量、缓存和测试中的相关说明；不得只改代码而保留与实现不符的注释，也不得留待后续任务整理。
- 符号、路径、数据格式、生命周期、执行顺序、边界或行为变化时，必须更新对应注释；失去用途的说明直接删除。相关注释未同步完成前，不得视为代码改动已完成。
